...
ndvi.vatten <- raster::extract(summer.scan, catchment.poly[which(catchment.poly$ebint == 16612216),], fun = mean, df = T, sp = T, na.rm = T)
```
...
map.ndvi <- ggplot() + geom_tile(data = ndvi.plot.spdf, aes(x = x, y = y, fill = layer, width = 10000, height = 10000))+
  scale_fill_distiller(type = "seq", palette = 2, direction = 1)+labs(fill = "Summer NDVI 1995")+
...
`r fig_nums("map_ndvi", "Map of summer NVDI in Norway, Sweden and Finland in 1994")`
...
`r fig_nums("map_clc", "Map of Land Cover in Norway, Sweden and Finland in 2000")`
...
`r table_nums("moran-table","Moran's I of the variables")`
...
`r table_nums("summary-lm-5","Results for the linear model with 5 selected predictors on Norway, Sweden and Finland")`
...
`r table_nums("summary-lm-5","Results for the spatial error linear model with 5 selected predictors on Norway, Sweden and Finland")`
...
`r fig_nums("sem-residuals","Map of residuals for a) scaled and b) unscaled SELM")`
...
`r fig_nums("compare-plot-5","Comparison of scaled estimates for LM and SELM (left) and of the effect sizes for each predictor (right) with NDVI, logRunoff, Bog, Arable and TNdep as predictors")`
...
`r table_nums("table-lm-9","Summary table for linear model with 9 predictors")`
...
`r fig_nums("lm-residuals-9","Map of residuals for a) scaled and b) unscaled LM with 9 predictors")`
...
`r table_nums("table-sem-9","Summary table for spatial error linear model with 9 predictors")`
...
`r fig_nums("residuals-sem-9","Map of residuals for a) scaled and b) unscaled SELM with 9 predictors")`
...
`r fig_nums("compare-plot-9","Comparison of scaled estimates for LM and SELM (left) and of the effect sizes for each predictor (right) with 9 predictors")`
...
`r table_nums("table-sem-3","Summary table for SELM model with 3 predictors")`
...
`r fig_nums("sem-residuals-3","Map of residuals for a) scaled and b) unscaled SELM with 3 predictors")`
...
`r table_nums("table-sem-precip","Summary table for SELM model with 3 predictors: NDVI, Bog and logPrecip")`
...
`r table_nums("table-sem-precip","Summary table for SELM model with 3 predictors: NDVI, TNdep and logPrecip")`
