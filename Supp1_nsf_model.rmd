...
```{r test-area}
g1 <- ggplot()+geom_point(data = subset(fennoscandia.33, area > 3500),aes(x=longitude, y = latitude, col = "big"), size = 1)+
  geom_point(data = subset(fennoscandia.33, area < 3500),aes(x=longitude, y = latitude, col = "small"), size = 2)+
...
```{r runoff, eval = F}
runoff.raster <- "CORDEX/mrro_Lmon_CNRM-CM6-1-HR_historical_r1i1p1f2_gr_185001-201412.nc"
...
saveRDS(runoff.fennoscandia,"CORDEX/total.runoff.fennoscandia.rds")
```

```{r total-runoff, eval = F}
...
```{r compare}
mrro <- readRDS("CORDEX/total.runoff.fennoscandia.rds") %>% setNames(c("ebint","mrro"))
mrros <- readRDS("CORDEX/runoff.fennoscandia.rds")%>% setNames(c("ebint","mrros"))
annual.prec <- readRDS("annual.prec.Rdata")
names(annual.prec) <- c("ebint","prec")

runoff <- merge(mrro, mrros, by = "ebint") %>% merge(annual.prec, by = "ebint") %>% st_as_sf()

r1 <- ggplot(runoff)+geom_point(aes(x=mrro, y = mrros))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
ggsave("CORDEX/surf.total.runoff.png", r1)

runoff$surf.total <- runoff$mrros / runoff$mrro * 100
r2 <- ggplot(runoff)+geom_sf(aes(fill=surf.total, col = surf.total))+scale_color_viridis_c(name = "Share of surface runoff in total runoff", option = "A", aesthetics = c("fill","col"))+
  theme_minimal()
ggsave("CORDEX/map.surf.total.png",r2)

r3 <- ggplot(runoff, aes(x = prec))+geom_point(aes(y = mrro, col = "total runoff"))+
  geom_point(aes(y = mrros, col = "surface runoff"))+scale_color_viridis_d(option = "A", end = 0.8)+
  annotate(label = paste("cor surface runoff/prec = ",round(cor(runoff$mrros, runoff$prec, use = "pairwise.complete"),2)), geom = "label", x = 2000, y = 1e-5)+
  annotate(label = paste("cor total runoff/prec = ",round(cor(runoff$mrro, runoff$prec, use = "pairwise.complete"),2)), geom = "label", x = 1000, y = 6e-5)+
  labs(y = "Runoff, kg/m2/s", x = "Precipitation, mm/y")
  theme_minimal()
ggsave("CORDEX/runoff.prec.png",r3)
