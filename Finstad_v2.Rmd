...
waterchem_1995_log$log_slope <- log10(waterchem_1995$slope + 0.001)
...
### Model with remote variables only
```{r model-remote}
#import raster, extract elevation value
library(raster)
elev_map <- raster("elevation/elevation1x1_new/elevation1x1_new.tif",varname = "elevation")
plot(elev_map)

elev_values <- NA
for (i in 1:dim(altitude.spdf)[1]){
elev_values[i] <- extract(elev_map, altitude.spdf[i,])
}

alt_map <- raster("elevation/elevation1x1_new/elevation1x1_new.tif",varname = "altitude")
plot(alt_map)

alt_value <- extract(alt_map,altitude.spdf[1,])

altitude.spdf <- SpatialPointsDataFrame(coords = data.frame(waterchem_1995$Longitude,waterchem_1995$Latitude), data = dplyr::select(waterchem_1995,toc), proj4string = CRS("+proj=longlat +ellps=WGS84"))
...
dep.var <- "std_log_toc"
ind.var <- c("std_ndvi","std_log_slope","std_log_runoff","std_tempr")
fm <- paste(dep.var,"~",paste(ind.var,collapse = "+"))
```
```{r hist-remote}
binwidth_hist <- function(x){
  if(IQR(x, na.rm = T) < 2){ 
...
for (i in 1:length(ind.var)){
  hi <- ggplot(waterchem_1995, aes_string(ind.var[i]))+geom_histogram(na.rm = TRUE, binwidth = function(x) max(x)/15)+labs(y = "", title = i)+theme_light()
  h_list[[ind.var[i]]] <- hi
}
...
lm.remote <- lm(fm, data = train_set)
summary(lm.remote)
...
bw.remote <- bw.gwr(fm, data = train_spdf, kernel = "gausian", adaptive = T)
model.selection.remote <- gwr.model.selection(DeVar = dep.var, InDeVars = ind.var, data = train_spdf,bw = bw.remote, kernel = "gaussian", adaptive = T)
gwr.model.view(DeVar = dep.var, InDeVars = ind.var, model.selection.remote[[1]])
gwr.remote <- gwr.basic(fm, data = train_spdf, bw = bw.remote, kernel = "gausian", adaptive = T)
# spatial autocorrelation of residuals?
```
```{r spatial-regression}
library(spatialreg)
lm.LMtests(lm.remote,k_neigh_w,c("LMerr","LMlag","RLMerr","RLMlag"))
lagsarlm(fm, train_set,k_neigh_w)
Wsparse <- as(k_neigh_w,"CsparseMatrix") %>% trW(type = "MC")
