...
save_plot("predictor.maps.png",all.map, ncol = 3, nrow = 3, base_height = 4, base_width = 3)
print(all.map)
```
...
moran.vif.df %>% arrange(desc(moran.I)) %>%
  ...
  kable_styling(bootstrap_options = "bordered", position = "center") %>% save_kable("moran.vif.table.png", self_contained = T)
```

```{r moran-fig}
list.grob <- c("map.TOC.png", "moran.vif.table.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
grid.arrange(list.grob, nrow = 1)
```
...
niva <- plot_grid(plotlist = listgrob, ncol= 2, nrow = 1, align = "v", greedy = T)
save_plot("niva.test.png",niva, ncol = 2, nrow = 1, base_height = 4, base_width = 3)
print(niva)
