...
<div align = 'center'>*`r fig_nums("niva-predict","Results of the TOC prediction. a) Observed vs. fitted logTOC, b) Map of residuals (observed- fitted)")`*</div>
...
```
<div align = 'center'>*`r fig_nums("niva-predict-lm","Results of the TOC prediction with Linear Model. a) Observed vs. fitted logTOC, b) Map of residuals (observed- fitted)")`*</div>
...
niva.data <- readRDS("NIVA_test/niva.data.rds")
niva.kmat <- readRDS("NIVA_test/niva.kmat.rds")
niva.data$logTOC <- log(niva.data$toc_2019)
...
selm.niva <- errorsarlm(fm,niva.data, niva.kmat)

fennoscandia <- readRDS("fennoscandia.rds")
k.neigh.w <- readRDS("k.neigh.w.rds")
fen_fit <- predict(selm.niva, newdata = fennoscandia, k.neigh.w)

fen_predict <- cbind(fennoscandia, fen_fit)
fen_predict$diffTOC <- fen_predict$TOC - exp(fen_predict$fit)
fen_predict$diffLogTOC <- fen_predict$logTOC - fen_predict$fit

saveRDS(fen_predict,"NIVA_test/fen_predict.rds")


```

```{r plot-reverse-model-results-selm, eval = F}
fen_predict <- readRDS("NIVA_test/fen_predict.rds")

ggplot(fen_predict) + geom_point(aes(y = logTOC, x = fen_fit, col = latitude))+
  labs(x = "logTOC 1995", y = "Predicted logTOC 1995")+
  scale_color_continuous_divergingx(palette = "RdYlBu", name = "Latitude", mid = median(fen_predict$latitude))+
  annotate(geom = "label", label = paste("r = ", round(cor(fen_predict$logTOC,fen_predict$fit),2)), x = 2, y = -0.2)+
  geom_abline(slope  = 1, intercept = 0, col = "red")+
  theme_minimal(base_size = 10)+theme(legend.position = "bottom")
ggsave(filename = "NIVA_test/fen_selm_result.png", dpi = "retina", width = 10, height = 12, units = "cm")
```