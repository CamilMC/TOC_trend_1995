...
clc.wr.arable <- select(clc.wr,c("V12","V16","V18","V19","V20")) %>% rowSums() %>% as.data.frame() %>% setNames("arable")
...
clc.wr.sum <- cbind(clc.wr.bogs, clc.wr.forest,clc.wr.arable,clc.wr.glacier, clc.wr.bare)
...
all.wr.sf <- merge(wr.sf,runoff.wr95,by = "gid") %>% merge(ndvi.wr,by="gid") %>% merge(clc.wr.sum,by = "gid") %>% merge(st_drop_geometry(tp),by = "gid") %>% merge(alt.wr, by = "gid") %>% merge(slope.wr, by = "gid") %>% merge(ndep.wr, by = "gid")
...
u <- c("NDVI on scale 0 to 1", "Mean Runoff, mm/y","Mean annual precipitation,\n mm/y","Mean annual temperature,\n °C","Proportion of bogs","Proportion of Forest","Proportion of\narable land","Total Nitrogen Deposition")
couleurs <- c("Greens","Blues","Blues","RdYlBu","YlOrBr","RdYlGn","RdYlGn","PuRd")
dir <- c(T,T,T,F,T,T,T,T)
...
summary(m1 <- step(m1))
plot(wr.sf.95$NDVI, predict(m1),  pch = ".", main="NDVI", xlab="Observed", ylab="Predicted",abline(0,1))
...
summary(m2 <- step(m2))
plot(wr.sf.95$NDVI, predict(m2),  pch = ".", main="NDVI", xlab="Observed", ylab="Predicted - quadratic",abline(0,1))

# poly 
summary(m3 <- lm(NDVI ~ poly(cbind(Temp, log(Precip)), degree = 5), data = wr.sf.95))
summary(m3 <- step(m3))
plot(wr.sf.95$NDVI, predict(m3),  pch = ".", main="NDVI", xlab="Observed", ylab="Predicted - poly ",abline(0,1))
```
...
poly.ndvi <- lm(NDVI ~ poly(cbind(Temp, log(Precip)), degree = 5), data = fennoscandia)
poly.ndvi <- step(poly.ndvi)
summary(poly.ndvi)
plot(fennoscandia$NDVI, poly.ndvi$fitted.values)

fennoscandia.ndvi.model <- fennoscandia.33 %>% st_as_sf() %>% mutate(ndvi.residuals = poly.ndvi$residuals) 

fen.residuals <- ggplot()+geom_sf(data = nor, fill = "white")+geom_sf(data = swe, fill = "white")+geom_sf(data = fin, fill = "white")+
  geom_sf(data = fennoscandia.ndvi.model, aes(fill = poly.ndvi$residuals, col = poly.ndvi$residuals))+
  scale_color_gradient2(high = "darkgreen", low = "red", mid = "white", midpoint = 0, aesthetics = c("fill", "col"))+
  theme_classic(base_size = 6)+theme(legend.position = "bottom")
ggsave(plot = fen.residuals, filename = "WR/poly.ndvi.residuals.plot.png",dpi = "retina", width = 6, height = 8, units = "cm")

wr.ndvi.test <- predict(object = poly.ndvi, newdata = wr.sf.95)
plot(wr.sf.95$NDVI, wr.ndvi.test)
wr.ndvi.model <- wr.sf.95 %>% mutate(ndvi.predicted = wr.ndvi.test, ndvi.residuals = wr.ndvi.test-wr.sf.95$NDVI)

ggplot(wr.ndvi.model)+geom_sf(aes(fill = ndvi.residuals, col = ndvi.residuals))+
...
ggsave(filename = "WR/wr.ndvi.residuals.plot.png",dpi = "retina", width = 6, height = 8, units = "cm")
```

```{r plot-residuals-ndvi}
listgrob <- list("WR/poly.ndvi.residuals.plot.png","WR/wr.ndvi.residuals.plot.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
grid.arrange(grobs = listgrob, ncol = 2, nrow = 1)
```
...
Predictions for future TNdep based on IPCC estimated emissions for NOx (summary for policymakers AR6):
* SSP1-2.6: 1/4 decrease in 2050, stable in 2100
* SSP3-7.0: x1.5 increase in 20150, x2 increase in 2100
...
wr.sf.126.2050 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(st_drop_geometry(ndvi.ssp126), by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp126.runoff)), by = "gid")

wr.sf.370.2050 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(st_drop_geometry(ndvi.ssp370), by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp370.runoff)), by = "gid")
...
wr.sf.126.2050$TNdep <- wr.sf.126.2050$TNdep95*3/4
wr.sf.370.2050$TNdep <- wr.sf.370.2050$TNdep95*1.5
...
wr.sf.126.2100 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(st_drop_geometry(ndvi.ssp126.2100), by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp126.runoff.2100)), by = "gid")

wr.sf.370.2100 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(st_drop_geometry(ndvi.ssp370.2100), by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp370.runoff.2100)), by = "gid")
...
wr.sf.126.2100$TNdep <- wr.sf.126.2100$TNdep95*3/4
wr.sf.370.2100$TNdep <- wr.sf.370.2100$TNdep95*2
...
```{r print-plot-forecast-temp, fig.dim = c(12,8)}
t1 <- readPNG("WR/wr.map.Temp.png") %>% rasterGrob
t2 <- readPNG("WR/SSP126.2050.diffTemp.png") %>% rasterGrob
t3 <- readPNG("WR/SSP126.2100.diffTemp.png") %>% rasterGrob

grid.arrange(t1, t2, t3, ncol = 3)

t0 <- readPNG("white.png") %>% rasterGrob
t4 <- readPNG("WR/SSP370.2050.diffTemp.png") %>% rasterGrob
t5 <- readPNG("WR/SSP370.2100.diffTemp.png") %>% rasterGrob

grid.arrange(t0, t4, t5, ncol = 3)
#knitr::include_graphics(c("WR/wr.map.Temp.png","WR/SSP126.2050.diffTemp.png","WR/SSP370.2050.diffTemp.png","WR/SSP126.2100.diffTemp.png","WR/SSP370.2100.diffTemp.png"))
```
...
listgrob <- list("WR/wr.map.Precip.png","WR/SSP126.2050.diffPrecip.png","WR/SSP126.2100.diffPrecip.png","white.png","WR/SSP370.2050.diffPrecip.png","WR/SSP370.2100.diffPrecip.png") %>% lapply(readPNG) %>% lapply(rasterGrob)

grid.arrange(grobs = listgrob, ncol = 3, nrow = 2)
```
...
listgrob <- list("WR/wr.map.NDVI.png","WR/SSP126.2050.diffNDVI.png","WR/SSP126.2100.diffNDVI.png","white.png","WR/SSP370.2050.diffNDVI.png","WR/SSP370.2100.diffNDVI.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
...
listgrob <- list("WR/wr.map.Runoff.png","WR/SSP126.2050.diffRunoff.png","WR/SSP126.2100.diffRunoff.png","white.png","WR/SSP370.2050.diffRunoff.png","WR/SSP370.2100.diffRunoff.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
...
listgrob <- list("WR/wr.map.TNdep.png","WR/SSP126.2050.diffTNdep.png","WR/SSP126.2100.diffTNdep.png","white.png","WR/SSP370.2050.diffTNdep.png","WR/SSP370.2100.diffTNdep.png") %>% lapply(readPNG) %>% lapply(rasterGrob)

grid.arrange(grobs = listgrob, ncol = 3, nrow = 2)
```

`r fig_nums("future-tndep","Forecast of mean total nitrogen deposition for the period 2041-2060 (i.e., 2050) and for the period 2081-2100 (i.e., 2100), and change in TNdep compared to the average for 1995 to 2050, under SSP 1-2.6 (2ºC target) and SSP 3-7.0 (realistic worst-case) scenarios.")`
...
palettes <- data.frame(diffTOC =  brewer.pal(name = "RdBu", n = 9)[c(1,5,9)],
                       diff.log = brewer.pal(name = "RdBu", n = 9)[c(1,5,9)],
                       exp.diff.log = brewer.pal(name = "RdBu", n = 9)[c(1,5,9)])
...
g <- ggplot(list.sf[["SSP126.2050"]])+
    geom_sf(aes(fill=diff.log,col=diff.log))+
    scale_fill_gradient2(low=palettes[3,1], mid= palettes[2,1], high = palettes[1,1],limits = c(-1,3), aesthetics = c("fill","col"),name="Difference log(TOC)\n SSP126 - 2050")+
...
# Sea watersheds: coastal darkening
...
Approximately matching the borders Norway/Sweden/Finland: include graphics from QGIS
...
ggplot(filter(toc.export.df, Diff == "D")) + geom_point(aes(x=Year, y = TOC_Tg, col = Model, shape= Model)) +
  geom_path(aes(x=Year, y = TOC_Tg, col = Model, shape= Model, group = Model), size = 0.3) +
...
ggplot(filter(toc.export.df, Diff == "A")) + geom_point(aes(x=Year, y = TOC_Tg, col = Model, shape= Model)) +
  geom_path(aes(x=Year, y = TOC_Tg, col = Model, shape= Model, group = Model), size = 0.3) +
  labs(x="Year", y= "TOC (Tg)")+
  scale_color_manual(values = c("firebrick4","steelblue4","darkorange"))+
  facet_grid(rows=vars(Nation))+
  geom_abline(slope = 0, intercept = 0, col = "firebrick4", size = 0.3)+
  theme_bw(base_size = 5)
ggsave(filename = "WR/TOC_export_abs.png", dpi = "retina", width = 8, height = 6, units = "cm")
