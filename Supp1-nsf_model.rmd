...
# map NDVI 
ndvi.plot <- (floor(year.scan)/10)/1000
plot(ndvi.plot)

wr.sf.95 <- readRDS("WR/wr.sf.95.rds")
ndvi.plot.sf <- ndvi.plot %>% as("SpatialPixelsDataFrame") %>% st_as_sf() %>% st_transform(st_crs(wr.sf.95)) 

map.ndvi <- ggplot() + geom_sf(data = ndvi.plot.sf, aes(col = layer), shape = 15)+
  scale_color_distiller(type = "seq", palette = 2, direction = 1)+labs(color = "Summer NDVI 1995")+
  labs(subtitle = "a)")+
  theme_void(base_size = 2)
ggsave(plot = map.ndvi, filename = "NDVI/map.ndvi.png", dpi = "retina", width = 6, height = 8, units = "cm")
...
map.ndvi <- ggplot() + geom_tile(data = ndvi.plot.spdf, aes(x = x, y = y, fill = layer, width = 10000, height = 10000))+
  scale_fill_distiller(type = "seq", palette = 2, direction = 1)+labs(color = "Summer NDVI 1995", subtitle = "a)")+
  theme_void(base_size = 8)
ggsave(plot = map.ndvi, filename = "NDVI/map.ndvi.png", dpi = "retina", units = "cm")
...
## Soil thickness
```{r soil-thickness, eval = F}
soil <- raster::raster("DAAC/average_soil_and_sedimentary-deposit_thickness.tif")
catchment.poly <- readRDS("catchment.poly.Rdata")
soil.df <- extract(soil, catchment.poly, sp = T, df = T, na.rm = T)
