...
The dataset is compiled from six different data sources.
...
2. The land-use data was extracted from the Corine land-cover database (http://www.eea.europa.eu/publications/COR0-landcover). To simplify the categories, the land cover classes were merged the following way:
  + woodland = agro_forestry_area + broad_leave_area + coniferous_forest_area + mixed forest aread
...
for (i in names(waterchem_1995)[-which(names(waterchem_1995) %in% c("date_mmdd", "Longitude","Latitude", "lake_name","lake_name2"))]){
  hi <- ggplot(waterchem_1995, aes_string(i))+geom_histogram(na.rm = TRUE, binwidth = function(x) max(x)/15)+ylab("")+theme_light()
  h_list[[i]] <- hi
}
...
plot(g_moran)
```
Log(SO~4~), log(NDVI), log(runoff), log(TOC), log(TOTN)  have a Moran's I superior to 0.5, indicating a high level of spatial autocorrelation [@Koh2020]. Log(totP) and log(Ca) have a Moran's I valye close to 0.5 as well. Howeger, Population, agriculture, bogs, shrubs, catchment area and water area in the catchment are not autocorrelated and are randomly diretributed.
...
*or because most of the covariates are spatially autocorrelated*. 
...
Step 1 and 2 are done with the log-transformed parameters (except for NDVI), and steps 3, 4 and 5 are done with standardized log-transformed parameters. 
...
```{r vif, eval = F}
vif_waterchem <- gwr.collin.diagno(formula = std_log_toc~std_log_totp+std_log_so4+std_log_catchm_area+std_log_bogs+std_log_runoff+std_ndvi+std_log_shrubs+std_log_agriculture, data = waterchem_1995_spdf, bw = bw_std, adaptive = T)

fmvif <- formula(paste("std_log_toc",paste(names(waterchem_1995_std)[-1],collapse = "+"),sep="~"))
                    
vif_waterchem2 <- gwr.collin.diagno(fmvif, data = waterchem_std_spdf, bw=bw_std,adaptive = T)

saveRDS(vif_waterchem,"vif_waterchem.Rdata")

readRDS("vif_waterchem.Rdata")

local_vif <- vif_waterchem$VIF %>% as.data.frame() %>% setNames(c("std_log_totp","std_log_so4","std_log_catchm_area","std_log_bogs","std_log_runoff","std_ndvi","std_log_shrubs","std_log_agriculture"))

for(i in names(local_vif)){
  f_plotspatial(waterchem_1995,var = local_vif[,i], plottitle = i)
}

vif_list <- c()

for (i in names(local_vif)){
  vif_list[[i]] <- f_plotspatial(waterchem_1995,local_vif[,i], plottitle = i)
}
     

vif_list
     [-which(names(corr_toc) %in% c("Latitude","Longitude"))]){
...
#bw_std < readRDS("bw_std.Rdata")
#gw_ss2 <- gwss(waterchem_1995_spdf,vars = c("std_log_toc","std_log_totp","std_log_so4","std_log_catchm_area","std_log_bogs","std_log_runoff","std_ndvi", "log_shrubs", "log_agriculture"),kernel = "gaussian",adaptive = T, bw = bw_std)
...
```{r map-gwss, fig.cap = "Maps of Pearson correlation coefficients between TOC and the explanatory variables", fig.dim = c(40,60)}
...
bw.gwr.selection <- bw.gwr(std_log_toc~std_log_totp + std_log_so4+std_log_catchm_area+std_log_bogs+std_log_runoff+std_ndvi+ std_log_shrubs+std_log_agriculture, data = waterchem_std_spdf, adaptive = T)

waterchem_gwr_selection <- gwr.model.selection(DeVar = "std_log_toc", InDeVars = c("std_log_totp","std_log_so4","std_log_catchm_area","std_log_bogs","std_log_runoff","std_ndvi","std_log_shrubs","std_log_agriculture"),data = waterchem_1995_spdf, bw = bw.gwr.selection, adaptive = T)
...
The GWR model was fitted with TOC as response variable and TOTP, SO~4~, runoff, ndvi and as explanatory variables. 
...
bw_std <- bw.gwr(std_log_toc~std_log_totp + std_log_so4+std_log_runoff+std_ndvi+std_log_shrubs + std_log_agriculture,data = waterchem_std_spdf,adaptive = T)
