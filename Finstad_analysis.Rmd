...
```{r correlations, fig.dim = c(10,10)}
...
```{r scatterplot_selection, fig.dim = c(20,20)}
...
waterchem_1995_spdf <- SpatialPointsDataFrame(waterchem_1995_full[c("Longitude","Latitude")],waterchem_1995)

#bw_gwss <- bw.gwss.average(waterchem_1995_spdf,vars=names(waterchem_1995),kernel = "gaussian", adaptive = T)
#gw_ss <- gwss(waterchem_1995_spdf,vars = c("toc","totp","so4","catchment_area_km2","woodland","bogs","runoff","ndvi_summer_lag1yr"),kernel = "gaussian",adaptive = T, bw = 100)
...
# computes bwr
gwr_std <- gwr.basic(std_log_toc~std_log_so4+std_log_catchm_area+std_log_bogs+std_log_runoff+std_ndvi,data = waterchem_std_spdf, regression.points = waterchem_std_spdf, bw = bw_std, adaptive = T)
...
# plots coefficients
#f_plotspatial(data = waterchem_1995,var=gwr_std$SDF$std_log_so4,plottitle = "coef so4")
#f_plotspatial(data = waterchem_1995,var=gwr_std$SDF$std_log_catchm_area,plottitle = "coef catch")
#f_plotspatial(data = waterchem_1995,var=gwr_std$SDF$std_log_bogs,plottitle = "coef bogs")
#f_plotspatial(data = waterchem_1995,var=gwr_std$SDF$std_log_runoff,plottitle = "coef runoff")
#f_plotspatial(data = waterchem_1995,var=gwr_std$SDF$std_ndvi,plottitle = "coef ndvi")

# plots residuals
#f_plotspatial(data=waterchem_1995,var=gwr_std$lm$residuals,plottitle = "residuals")
# qplot(gwr_std$lm$fitted.values,gwr_std$lm$residuals,xlab = "Predicted", ylab = "Residuals") + geom_hline (yintercept = 0,col = "gray") + 
...
#    theme_bw(base_size = 12) + theme(panel.grid = element_line(color="gray95"))
