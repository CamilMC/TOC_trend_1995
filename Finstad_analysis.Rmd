...
```
```{r histograms-variables, fig.cap = "histograms"}
binwidth_hist <- function(x){
  if(IQR(x)==0){ 
    bw <- length(x)
  }else{
...
for (i in names(waterchem_1995)[-1]){
  data <- waterchem_1995[,i] %>% as.data.frame()
  hi <- ggplot(data)+geom_histogram(aes_string(i), binwidth = binwidth_hist(data[,1]))
  print(hi)
}
...
Moran's I of residuals: 0.0989, p-value = 0
Residuals are not spatially correlated. OLS could have some explanatory value? 
