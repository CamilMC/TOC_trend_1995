...
ggplot(ols_res)+geom_point(aes(x=long,y=lat,col=residuals))+theme_void()+scale_colour_gradient2(low="#4575B4",mid="#FFFFBF",high = "#D73027",midpoint = 0)+borders("Norway")
...
```{r spatial-autocorrelation, fig.cap = "first"}
...
# bw_gwss <- bw.gwss.average(waterchem_1995_spdf,vars=c("toc","totp","so4","catchment_area_km2","woodland","bogs","runoff","ndvi_summer_lag1yr"),kernel = "gaussian", adaptive = T)
...
shinyApp(ui = ui_2, server = server_2, options = list(height = 200))
...
shinyApp(ui = ui_3, server = server_3, options = list(height = 200))
```
