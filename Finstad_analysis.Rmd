...
```{r all-map-gwss, fig.cap = "Maps of Pearson correlation coefficients", fig.dim = c(50,250),eval = F}
...
gwss_table <- data.frame(sapply(gwss_pearson,max),sapply(gwss_pearson,min))
```
#### CN
Large condition number indicates high collinearity between the columns of a given matrix @Wheeler2007. A condition number higher then 30 is considered as a threshold for collinerity. In the model with all variables, all local CN are much greater than 30. 

Use gaussian kernel @Warsito2018

```{r local-collinearity-formula, eval = T}
...
```{r local-collinerarity, eval = F}

bw_vif <- bw.gwr(fmvif,train_spdf[,-c(1:2)],kernel = "gaussian", adaptive =T)

vif_waterchem <- gwr.collin.diagno(formula = fmvif, data = train_spdf, bw = bw_vif, kernel = "gaussian", adaptive = T)

saveRDS(vif_waterchem,"vif_waterchem.Rdata")

```

```{r condition-number, eval = T}
vif_waterchem <- readRDS("vif_waterchem.Rdata")
local_cn <- vif_waterchem$local_CN %>% as.data.frame() %>% setNames("CN")
cn_plot <- f_plotspatial(train_set,local_cn$CN,plottitle = "Local CN", midpoint = 30, lim = c(0,max(local_cn)))
plot(cn_plot)
```
...
```{r vif-table}
vif_max_sorted <- sapply(local_vif,max) %>% sort(decreasing = T)
vif_mean_sorted <- sapply(local_vif,mean) %>% sort(decreasing = T)
vif_median_sorted <- sapply(local_vif,median) %>% sort(decreasing = T)

vif_sorted <- data.frame(round(vif_max_sorted,2),round(vif_mean_sorted,2),round(vif_median_sorted,2)) %>% setNames(c("max","mean","median")) # are variables disordered?

knitr::kable(vif_sorted, caption = "Summary of vif value for predictor variables")
```
...
```
```{r vdp-table}
vdp_max_sorted <- sapply(local_vdp,max) %>% sort(decreasing = T)
vdp_mean_sorted <- sapply(local_vdp,mean) %>% sort(decreasing = T)
vdp_median_sorted <- sapply(local_vdp,median) %>% sort(decreasing = T)

vdp_sorted <- data.frame(round(vdp_max_sorted,2),round(vdp_mean_sorted,2),round(vdp_median_sorted,2)) %>% setNames(c("max","mean","median"))

knitr::kable(vdp_sorted, caption = "Summary of vdp value for predictor variables")
```

### Collinearity after selection of variables

```{r vif_selected}
fmvif2 <- std_log_toc~std_log_totp+std_log_so4+std_log_shrubs+std_log_water+std_log_bogs+std_log_runoff+std_log_population+std_ndvi
...
vif_waterchem2 <- gwr.collin.diagno(formula = fmvif2, data = train_spdf, bw = bw_vif2, kernel = "gaussian", adaptive = T)

saveRDS(vif_waterchem2,"vif_waterchem2.Rdata")

vif_waterchem2 <- readRDS("vif_waterchem2.Rdata")
```
```{r condition-number2, eval = F}
local_cn2 <- vif_waterchem2$local_CN %>% as.data.frame() %>% setNames("CN")
cn_plot2 <- f_plotspatial(train_set,local_cn2$CN,plottitle = "Local CN", midpoint = 30, lim = c(0,max(local_cn2)))
plot(cn_plot2)
```
```{r map-vif2}
...
vif2_list <- lapply(local_vif2,f_plotspatial,data = train_set,plottitle = "",lim = c(min(local_vif2),max(local_vif2)),midpoint = 4)

names(vif2_list) <- names_vif2

for(i in names_vif2){
  vif2_list[[i]] <- vif2_list[[i]]+ggtitle(i)
}

vif2_grobs <- lapply(vif2_list,ggplotGrob)

grobc.plot2 <- arrangeGrob(grobs = vif2_grobs,ncol = 2)
grid.draw(grobc.plot2)
```

```{r table-vif2}
vif2_mean_sorted <- sapply(local_vif2,mean) %>% sort(decreasing = T)
vif2_median_sorted <- sapply(local_vif2,median) %>% sort(decreasing = T)
vif2_max_sorted <- sapply(local_vif2,max) %>% sort(decreasing = T)

vif2_sorted <- data.frame(round(vif2_max_sorted,2),round(vif2_mean_sorted,2),round(vif2_median_sorted,2)) %>% setNames(c("max","mean","median"))

knitr::kable(vif2_sorted, caption = "Summary of vif value for selected predictor variables")
```

```{r map-vdp-selected}
local_vdp2 <- vif_waterchem2$VDP %>% as.data.frame() %>% setNames(c("intercept",names_vif2))

vdp2_list <- lapply(local_vdp2,f_plotspatial,data = train_set, plottitle = "", lim = c(min(local_vdp),max(local_vdp)),midpoint = 0.5)

for(i in names(vdp2_list)){
  vdp2_list[[i]] <- vdp2_list[[i]]+ggtitle(i)
}

vdp2_grobs <- lapply(vdp2_list,ggplotGrob)

vdp.plot2 <- arrangeGrob(grobs = vdp2_grobs,ncol = 2)
grid.draw(vdp.plot2)
```

```{r table-vdp-selectd}
vdp2_median_sorted <- sapply(local_vdp2,median) %>% sort(decreasing = T)
vdp2_max_sorted <- sapply(local_vdp2,max) %>% sort(decreasing = T)
vdp2_mean_sorted <- sapply(local_vdp2,mean) %>% sort(decreasing = T)
  
vdp2_sorted <- data.frame(round(vdp2_max_sorted,2),round(vdp2_mean_sorted,2),round(vdp2_median_sorted,2)) %>% setNames(c("max","mean","median"))

knitr::kable(vdp2_sorted, caption = "Summary of vdp value for selected predictor variables")
```
The VDP calculated with the selected variables are higher than the VDP calculated with all variables. But the mean VDP values are under 0.5 so the selected variables are kept for the analysis. 
...
```{r GWR-model-selection, fig.cap = "Model selection for GWR", results = "hide", eval = T}

bw.gwr.selection <- bw.gwr(fmvif2, data = train_spdf, adaptive = T,kernel = "gaussian")

waterchem_gwr_selection <- gwr.model.selection(DeVar = "std_log_toc", InDeVars = c("std_log_totp","std_log_so4","std_log_shrubs","std_log_water","std_log_bogs","std_log_runoff","std_log_population","std_ndvi"),data = train_spdf, bw = bw.gwr.selection, adaptive = T,kernel = "gaussian")

gwr.model.view("std_log_toc",c("std_log_totp","std_log_so4","std_log_shrubs","std_log_water","std_log_bogs","std_log_runoff","std_log_population","std_ndvi"),waterchem_gwr_selection[[1]])
...
bw.montecarlo <- bw.gwr(std_log_toc~std_log_totp + std_ndvi + std_log_runoff + std_log_so4, train_spdf,adaptive = T, kernel = "gaussian")
train_montecarlo <- gwr.montecarlo(std_log_toc~std_log_totp + std_ndvi + std_log_runoff + std_log_so4,data = train_spdf,adaptive = T,kernel = "gaussian", bw = bw_std)
saveRDS(train_montecarlo,file = "train_gwr_montecarlo.Rdata")
```

```{r montecarlo-results, results = "asis", eval = T}
train_montecarlo_load <- readRDS("train_gwr_montecarlo.Rdata", )
knitr::kable(train_montecarlo_load, caption = "Monte-Carlo significance test results")
```
...
coord_mat <- train_set[,c("Longitude","Latitude")] %>% as.matrix()

dist_mat <- gw.dist(coord_mat,coord_mat)
# selects bandwidth

bw_std <- bw.gwr(std_log_toc~std_log_totp + std_log_so4+std_log_runoff+std_ndvi+std_log_shrubs + std_log_agriculture,data = train_spdf,adaptive = T,dMat = dist_mat)
saveRDS(bw_std,"bw_std.Rdata")

# computes bwr
gwr_std <- gwr.robust(std_log_toc~std_log_totp + std_log_so4+std_log_runoff+std_ndvi+std_log_shrubs + std_log_agriculture,data = train_spdf, dMat = dist_mat, bw = bw_std, adaptive = T)
...
```
However, the Moran's I of the residuals is **`r round(moran_gwr_res$estimate[1],2)`** and its p-value is **`r moran_gwr_res$p.value`**. The value is close to zero, showing that the residuals are not autocorrelated and are randomly distributed. 
...
predict_test <- gwr.predict(std_log_toc~std_log_totp + std_log_so4+std_log_runoff+std_ndvi+std_log_shrubs + std_log_agriculture,test_spdf,bw = bw_std)
...
### Alternative variable selection with VIF 
...
fm1 <- paste(vif.selected.variables[1],"~",paste(vif.selected.variables[-1],collapse="+")) %>% as.formula(env = parent.frame())
bw1.gwr <- bw.gwr(fm1,data = train_spdf, adaptive = T, kernel = "gaussian")
gwr1 <- gwr.basic(fm1,data=train_spdf,bw=bw1.gwr,kernel = "gaussian",adaptive = T)

gwr1.res <- gwr1$SDF@data
f_plotspatial(train_set,gwr1.res$std_log_bogs, plottitle = "bogs")
f_plotspatial(train_set,gwr1.res$std_log_runoff, plottitle = "runoff")
f_plotspatial(train_set,gwr1.res$Local_R2,plottitle = "R2")
f_plotspatial(train_set,gwr1.res$Stud_residual,plottitle = "Residuals")

AIC(gwr1$lm)
gwr1$GW.diagnostic$AICc

```
The selected variables are `r vif.selected.variables`.

# Alternative varibale selection with VDP
Same method, but removing the variable with the highest VDP. 
...
fm2 <- paste(vdp.selected.variables[1],"~",paste(vdp.selected.variables[-1],collapse="+")) %>% as.formula(env = parent.frame())
bw2.gwr <- bw.gwr(fm2,data = train_spdf, adaptive = T, kernel = "gaussian")
gwr2 <- gwr.basic(fm2,data=train_spdf,bw=bw2.gwr,kernel = "gaussian",adaptive = T)

gwr2.res <- gwr2$SDF@data
f_plotspatial(train_set,gwr2.res$std_log_shrubs, plottitle = "shrubs")
f_plotspatial(train_set,gwr2.res$std_log_water, plottitle = "water")
f_plotspatial(train_set,gwr2.res$Local_R2,plottitle = "R2")
f_plotspatial(train_set,gwr2.res$Stud_residual,plottitle = "Residuals")

```

The selected variables are `r vdp.selected.variables`. 
...
fit.gwr <- function(variables, spdf, df){
...
fit.gwr(c("std_log_toc","std_ndvi"),train_spdf,train_set)
fit.gwr(c("std_log_toc","std_log_runoff"), train_spdf,train_set)
fit.gwr(c("std_log_toc","std_log_so4"), train_spdf,train_set)
fit.gwr(c("std_log_toc","std_log_population"), train_spdf,train_set)
...
#### Robust gwr
@DeBellefon2018
intercept = reference value

test non stationarity of coefficients using monte carlo
problem of collinearity when the average of the VIF is >2

```{r robust-gwr, eval = F}
gw.dist 
bw.gwr
gwr.robust with pre-calculated distance matrix
