...
toplot <- names(waterchem_1995)[-which(names(waterchem_1995) %in% c("date_mmdd", "Longitude","Latitude", "lake_name","lake_name2","waterBodyID_uncertain"))]
...
#bw_gwss <- bw.gwss.average(train_spdf,vars=c("std_log_toc","std_log_totp","std_log_sdep","std_log_catchm_area","std_log_bogs","std_log_runoff","std_ndvi"),kernel = "gaussian", adaptive = T)
#bw_std < readRDS("bw_std.Rdata")
#gw_ss2 <- gwss(train_spdf,vars = c("std_log_toc","std_log_totp","std_log_sdep","std_log_catchm_area","std_log_bogs","std_log_runoff","std_ndvi", "log_shrubs", "log_agriculture"),kernel = "gaussian",adaptive = T, bw = bw_std)
...
server <- function(input,output){
  output$map_toc <- renderPlot({
    f_plotspatial(data = train_set, var = waterchem_gwss_toc[,input$correlation_toc], plottitle = input$correlation)
  })
}
...
local_vif$lat <- train_set$Latitude
local_vif$long <- train_set$Longitude

local_vdp <- vif_waterchem$VDP %>% as.data.frame() %>% setNames(c(names_vif,"unknown"))
```

```{r map-vif, fig.dim = c(8,16), fig.cap = 'vif'}
vif_list <- lapply(local_vif,f_plotspatial,data = train_set,plottitle = "")
...
vdp_list <- lapply(local_vdp,f_plotspatial,data = train_set,plottitle ="")

names(vdp_list)

for(i in names(local_vdp)){
  f_plotspatial(train_set,var = local_vdp[,i], plottitle = i)
}

vdp_list <- list()

for (i in names(local_vdp)){
  vdp_list[[i]] <- f_plotspatial(train_set,local_vdp[,i], plottitle = i)
}
     
vdp_list[-which(names(vdp_list) %in% c("Latitude","Longitude"))]
...
```{r map-residuals-GWR, fig.cap= "Map of residuals for GWR TOC model", fig.dim = c(5,4)}
