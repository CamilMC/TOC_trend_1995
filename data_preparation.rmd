...
fennoscandia <- readRDS("fennoscandia_raw")
...
proj4string(fennoscandia.spdf) <- CRS("+proj=longlat +datum=WGS84")
fennoscandia.33 <- spTransform(fennoscandia.spdf,CRS("+proj=utm +zone=33N +datum=WGS84")) %>% st_as_sf()
ggplot(fennoscandia.33)+geom_sf(aes(fill = ndvi,col = ndvi))
```