...
fennoscandia <- fennoscandia %>% filter(mean.runoff > 0)
...
for (i in names(fennoscandia[,-c(1:2)])){
  if(IQR(fennoscandia[[i]], na.rm = T) > 0.1){
    bi <- ggplot(data = fennoscandia,aes_string(i))+geom_boxplot(stat = "bin",na.rm = T,binwidth = function(x) 2*IQR(x, na.rm = T)/(length(x)^(1/3)))+labs(y="",title = i)+theme_light()
  }else{
    bi <- ggplot(data = fennoscandia,aes_string(i))+geom_boxplot(stat = "bin",na.rm = T)+labs(y="",title = i)+theme_light()
  }
  b_list[[i]] <- bi
}
...
fennoscandia$log.toc <- log(fennoscandia$toc_mg_l) 
...
fennoscandia$log.runoff <- log(fennoscandia$runoff)  
...
fennoscandia$log.prec <- log(fennoscandia$prec+1e-5)
