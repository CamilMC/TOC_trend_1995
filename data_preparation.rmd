...
summer.mean.2015 <- raster::stack(summer.scandinavia.2015[[6]],summer.scandinavia.2015[[7]],summer.scandinavia.2015[[8]]) %>% mean()

summer.scan <- reclassify(summer.scandinavia.2015, cbind(-Inf, 0, NA), right=FALSE)

summer.ndvi.2015 <- raster::extract(summer.scan,catchment.poly, fun = mean, df = T, sp = T)
names(summer.ndvi.2015) <- c("ebint","ndvi2015")
summer.ndvi.2015$ndvi.value <- floor(summer.ndvi.2015$ndvi/10)/1000

saveRDS(summer.ndvi.2015, "summer.ndvi.2015.Rdata")
write.csv(summer.ndvi.2015,"summer.ndvi.2015.csv")
```
...
v <- c("toc_mg_l","runoff","temp","prec","bogs","arable","forest","tndep")
names(fennoscandia)[which(names(fennoscandia) %in% v)] <- c("TOC", "Runoff","Temp","Precip","Bog","Forest","Arable","TNdep")
...
fennoscandia$s.Bogs <- scale(fennoscandia$Bog)
