...
`r fig_nums("map-ndvi", "Summer NDVI in 1994 in coastal drainage basins")`
...
`r fig_nums("map-runoff","Runoff in 1995 in coastal drainage basins (mm/y)")`
...
`r fig_nums("map_clc","Proportion of a) bogs and b) arable land in 2000 in coastal drainage basins (%)")`
...
`r fig_nums("map-temp-precip", "Map of a) average temperature (°C) and b) average precipitation (mm/y) in 1995 in coastal drainage basins ")`
...
`r fig_nums("map-tndep","Total nitrogen deposition in 1995 in coastal drainage basins (mg/m2)")`
...
`r fig_nums(name = "fit-toc-95", caption = "Map of log(TOC) in coastal drainage basins for 1995")`
...
`r fig_nums(name = "future-temp", caption = "Map of forecasted temperature (ºC) for 2050 and 2100, under SSP 1-2.6 and SSP 3-7.0")`
...
`r fig_nums(name = "future-prec", caption = "Map of forecasted precipitations (mm/s) for 2050 and 2100, under SSP 1-2.6 and SSP 3-7.0")`
...
summary(m1 <- step(m1))
plot(wr.sf.95$NDVI, predict(m1),  pch = ".", main="a ) NDVI - LM", xlab="Observed NDVI", ylab="Predicted",abline(0,1))
...
plot(wr.sf.95$NDVI, ndvi.model$fitted.value,  pch = ".", main="b) NDVI - SELM", xlab="Observed", ylab="Predicted",abline(0,1))
...
summary(m2 <- step(m2))
plot(wr.sf.95$NDVI, predict(m2),  pch = ".", main="c) NDVI - quadratic model", xlab="Observed", ylab="Predicted",abline(0,1))
...
summary(m3 <- step(m3))
plot(wr.sf.95$NDVI, predict(m3),  pch = ".", main="d) NDVI - polynomial ", xlab="Observed", ylab="Predicted",abline(0,1))
text(x = 0.8, y = 0.4, label = paste("r2 =", round(summary(m3)$adj.r.squared,2), sep= ""))

saveRDS(m3,"WR/poly.ndvi.rds")
```

`r fig_nums("ndvi-model-test","Test of 4 NDVI models, a) linear model, b) spatial error linear model, c) quadratic model, d) polynomial model")`
...
ggplot(fennoscandia.selm.ndvi.model) + geom_point(aes(y = ndvi.fit, x = NDVI, col = latitude), size = 1)+
   ...
   labs(x = "NDVI", y = "Predicted NDVI",subtitle = "a)")+
...
ggplot(fennoscandia.poly.ndvi.model) + geom_point(aes(y = ndvi.fit, x = NDVI, col = latitude), size = 1)+
   scale_color_distiller(type = "div", palette = "RdYlBu", direction = 1, aesthetics = c("col"))+
   labs(x = "NDVI", y = "Predicted NDVI",subtitle = "c)")+
...
plot_grid(plotlist = listgrob, ncol = 2, nrow = 2, rel_heights = c(1,2), labels = c("a) Observed vs predicted NDVI for SELM","b) Observed vs predicted NDVI for polynomial model","c) Map of residuals for SELM","d) Map of residuals for Polynomial model"))
```

`r fig_nums(name = "residuals-ndvi", caption = "Results of NDVI models (SELM and polynomial): observed vs predicted for a) SELM and b) polynoimal model,  and map of residuals (observed - predicted) for c) SELM and d) Polynomial")`
...
`r fig_nums(name = "residuals-ndvi", caption = "Map of residuals of log(TOC) in coastal drainage basins for 1995")`
...
`r fig_nums("toc-export","TOC exported from coastal drainage basins (mg/L)")`
