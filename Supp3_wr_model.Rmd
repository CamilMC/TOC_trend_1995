...
```{r ndvi-model-test, fig.dim = c(10,10)}
library(betareg)
library(VGAM)
...
# SELM
# ndvi.model <- errorsarlm(formula = NDVI~Temp+log(Precip), data = fennoscandia, listw = fen.kmat)
# saveRDS(ndvi.model, "WR/selm.ndvi.rds")

ndvi.model <- readRDS("WR/selm.ndvi.rds")
...
ggplot(wr.selm.ndvi.model) + geom_point(aes(y = ndvi.fit, x = NDVI, col = Latitude), size = 1)+
   ...
   annotate(geom = "label", label = paste("r = ", round(cor(wr.selm.ndvi.model$NDVI,wr.selm.ndvi.model$ndvi.fit)^2,2)), x = 0.75, y = 0.3, size = 2)+
   geom_abline(slope  = 1, intercept = 0, col = "red")+
  labs(title = "Observed vs predicted NDVI for SELM")+
   theme_minimal(base_size = 6)
ggsave(filename = "WR/wr.ndvi.selm.results.png", dpi = "retina", width = 6, height = 8, units = "cm")
...
ggsave(plot = wr.residuals.selm, filename = "WR/wr.selm.ndvi.residuals.png",dpi = "retina", width = 6, height = 8, units = "cm")
```
...
ggplot(wr.poly.ndvi.model) + geom_point(aes(y = ndvi.fit, x = NDVI, col = Latitude), size = 1)+
   scale_color_distiller(type = "div", palette = "RdYlBu", direction = 1, aesthetics = c("col"))+
   labs(x = "NDVI", y = "Predicted NDVI")+
   annotate(geom = "label", label = paste("r = ", round(cor(wr.poly.ndvi.model$NDVI,wr.poly.ndvi.model$ndvi.fit),2)), x = 0.75, y = 0.3, size = 2)+
   geom_abline(slope  = 1, intercept = 0, col = "red")+
  labs(title = "Observed vs predicted NDVI for polynomial model")+
   theme_minimal(base_size = 6)+
  labs(title = "Map of residuals for Polynomial model")
ggsave(filename = "WR/wr.ndvi.poly.results.png", dpi = "retina", width = 6, height = 8, units = "cm")
...
wr.residuals <- ggplot()+
  ...
  theme_minimal(base_size = 6)+theme(legend.position = "bottom")
ggsave(plot = wr.residuals, filename = "WR/wr.poly.ndvi.residuals.png",dpi = "retina", width = 6, height = 8, units = "cm")
...
ggplot(wr.glm.ndvi.model) + geom_point(aes(y = ndvi.fit, x = NDVI, col = Latitude), size = 1)+
   scale_color_distiller(type = "div", palette = "RdYlBu", direction = 1, aesthetics = c("col"))+
   labs(x = "NDVI", y = "Predicted NDVI")+
   annotate(geom = "label", label = paste("r = ", round(cor(wr.glm.ndvi.model$NDVI,wr.glm.ndvi.model$ndvi.fit),2)), x = 0.25, y = 0.5, size = 2)+
   geom_abline(slope  = 1, intercept = 0, col = "red")+
  labs(title =  "Observed vs predicted NDVI for binomial model")+
   theme_minimal(base_size = 6)
ggsave(filename = "WR/wr.ndvi.glm.results.png", dpi = "retina", width = 6, height = 8, units = "cm")
...
ggsave(plot = wr.residuals, filename = "WR/wr.glm.ndvi.residuals.png",dpi = "retina", width = 6, height = 8, units = "cm")
```




```{r plot-residuals-ndvi, fig.dim = c(15,5)}
listgrob1 <- list("WR/wr.ndvi.selm.results.png","WR/wr.ndvi.poly.results.png","WR/wr.ndvi.glm.results.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
plot_grid(plotlist = listgrob1, ncol = 3, nrow = 1, labels = c("a)","b)","c)"))

```

```{r plot-res-ndvi, fig.dim ) c(12,16)}
listgrob2 <- list("WR/wr.selm.ndvi.residuals.png","WR/wr.poly.ndvi.residuals.png", "WR/wr.glm.ndvi.residuals.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
plot_grid(plotlist = listgrob2, ncol= 3, nrow = 1,labels = c("d)","e)","f)"), hjust = 3)
```
...
ggplot(mtrx.melt,aes(x=Temp, y = logPrecip, z = NDVI)) + stat_contour(geom="polygon", aes(fill = ..level..))+
  geom_tile(aes(fill = NDVI))+
  stat_contour(bins = 15)+
  geom_point(data = fennoscandia, aes(x=Temp, y = logPrecip), size = 1)+
...
ggplot(mtrx.melt1,aes(x=Temp, y = logPrecip, z = NDVI)) + stat_contour(geom="polygon", aes(fill = ..level..))+
  geom_tile(aes(fill = NDVI))+
  stat_contour(bins = 15)+
...
ggplot(mtrx.melt2,aes(x=Temp, y = logPrecip, z = NDVI)) + stat_contour(geom="polygon", aes(fill = ..level..))+
  geom_tile(aes(fill = NDVI))+
  stat_contour(bins = 15)+
...
ggplot(mtrx.melt3,aes(x=Temp, y = logPrecip, z = NDVI)) + stat_contour(geom="polygon", aes(fill = ..level..))+
  geom_tile(aes(fill = NDVI))+
  stat_contour(bins = 15)+
...
ggplot(mtrx.melt4,aes(x=Temp, y = logPrecip, z = NDVI)) + stat_contour(geom="polygon", aes(fill = ..level..))+
  geom_tile(aes(fill = NDVI))+
  stat_contour(bins = 15)+
...
```{r plot-forecast.log.toc.png}
knitr::include_graphics("forecast.log.toc.png")
```
...
```{r forecast.percent.change.toc}
knitr::include_graphics("forecast.percent.change.toc.png")
```
