...
all.wr.sf <- merge(wr.sf,runoff.wr95,by = "gid") %>% merge(ndvi.wr,by="gid") %>% merge(bogs.wr,by = "gid") %>% merge(forest.wr, by = "gid") %>% merge(arable.wr, by = "gid") %>%  merge(bare.wr, by = "gid") %>% merge(st_drop_geometry(tp),by = "gid") %>% merge(alt.wr, by = "gid") %>% merge(slope.wr, by = "gid") %>% merge(ndep.wr, by = "gid")
...
toreplace <- which(names(wr.sf.95) %in% c("ndvi","bogs","forest","arable","bare","temp","prec","alt","slope","tndep"))
names(wr.sf.95)[toreplace] <- c("NDVI","Bog","Forest","Arable","Bare","Temp","Precip","Alt","Slope","TNdep")
...
# SSP126
df.ssp126 <- dplyr::select(wr.sf.95, c("gid","area","Alt","Slope","Latitude","Bare")) %>% merge(ssp126.temp.wr, by = "gid") %>% merge(ssp126.prec.wr, by = "gid") %>% setNames(c("gid","area","Alt","Slope","Latitude","Bare","Temp","Precip")) 
saveRDS(df.ssp126, "WR/df.ssp126.rds")

# SSP 370
df.ssp370 <- dplyr::select(wr.sf.95, c("gid","area","Alt","Slope","Latitude","Bare")) %>% merge(ssp370.temp.wr, by = "gid") %>% merge(ssp370.prec.wr, by = "gid") %>% setNames(c("gid","area","Alt","Slope","Latitude","Bare","Temp","Precip")) 
saveRDS(df.ssp370, "WR/df.ssp370.rds")

# SSP126 2100
df.ssp126.2100 <- dplyr::select(wr.sf.95, c("gid","area","Alt","Slope","Latitude","Bare")) %>% merge(ssp126.temp.2100, by = "gid") %>% merge(ssp126.prec.2100, by = "gid") %>% setNames(c("gid","area","Alt","Slope","Latitude","Bare","Temp","Precip")) 
saveRDS(df.ssp126.2100, "WR/df.ssp126.2100.rds")

# SSP 370 2100
df.ssp370.2100 <- dplyr::select(wr.sf.95, c("gid","area","Alt","Slope","Latitude","Bare")) %>% merge(ssp370.temp.2100, by = "gid") %>% merge(ssp370.prec.2100, by = "gid") %>% setNames(c("gid","area","Alt","Slope","Latitude","Bare","Temp","Precip")) 
...
# glm
library(mgcv)
summary(m4 <- glm(NDVI ~ Temp+sqrt(Bare), data = wr.sf.95, family = "binomial"))
summary(m4 <- step(m4))
plot(predict(m4, type = "response"),  wr.sf.95$NDVI, pch = ".", main="e) NDVI - glm ", ylab="Observed", xlab="Predicted",abline(0,1))
text(x = 0.8, y = 0.4, label = paste("r =", cor(wr.sf.95$NDVI, m4$fitted.values), sep= ""))
...
```{r poly-ndvi-model-plot}
poly.ndvi <- readRDS("WR/poly.ndvi.rds")

poly.ndvi.fen <-  predict(poly.ndvi,fennoscandia.33, type = "response")
...
```{r glm-NDVI-model-plot}

glm.ndvi <- readRDS("WR/glm.ndvi.rds")
fennoscandia.33$Latitude <- fennoscandia.33$latitude

glm.ndvi.fen <-  predict(glm.ndvi,fennoscandia.33, type = "response")
fennoscandia.glm.ndvi.model <- fennoscandia.33 %>% mutate(ndvi.fit = glm.ndvi.fen, ndvi.residuals = fennoscandia.33$NDVI - glm.ndvi.fen) 
fennoscandia.glm.ndvi.model$area <- st_area(fennoscandia.selm.ndvi.model) %>% as.numeric()

ggplot(fennoscandia.glm.ndvi.model) + geom_point(aes(y = ndvi.fit, x = NDVI, col = latitude), size = 1)+
   scale_color_distiller(type = "div", palette = "RdYlBu", direction = 1, aesthetics = c("col"))+
   labs(x = "NDVI", y = "Predicted NDVI")+
   annotate(geom = "label", label = paste("r = ", round(cor(fennoscandia.glm.ndvi.model$NDVI,fennoscandia.glm.ndvi.model$ndvi.fit),2)), x = 0.75, y = 0.3, size = 2)+
   geom_abline(slope  = 1, intercept = 0, col = "red")+
   theme_minimal(base_size = 6)
ggsave(filename = "WR/fen.ndvi.glm.results.png", dpi = "retina", width = 6, height = 8, units = "cm")

fen.residuals <- ggplot()+
  geom_sf(data = fennoscandia.glm.ndvi.model, aes(fill = ndvi.residuals, col = ndvi.residuals))+
  scale_colour_continuous_divergingx(palette = "RdYlBu", mid = 0, aesthetics = c("fill", "col"), name = "glm residuals")+
  theme_minimal(base_size = 6)+theme(legend.position = "bottom")
ggsave(plot = fen.residuals, filename = "WR/fen.glm.ndvi.residuals.png",dpi = "retina", width = 6, height = 8, units = "cm")

```

```{r plot-residuals-ndvi, fig.dim = c(12,16)}
listgrob <- list("WR/fen.ndvi.selm.results.png","WR/fen.ndvi.poly.results.png","WR/fen.ndvi.glm.results.png","WR/fen.selm.ndvi.residuals.png","WR/fen.poly.ndvi.residuals.png", "WR/fen.glm.ndvi.residuals.png") %>% lapply(readPNG) %>% lapply(rasterGrob)
plot_grid(plotlist = listgrob, ncol = 3, nrow = 2, rel_heights = c(1,2), labels = c("a) Observed vs predicted NDVI for SELM","b) Observed vs predicted NDVI for polynomial model","c) Observed vs predicted NDVI for binomial model","d) Map of residuals for SELM","e) Map of residuals for Polynomial model","f) Map of residuals for Binomial model"), hjust = 3)
```
...
ndvi.model <- readRDS("WR/glm.ndvi.rds")
wr.kmat <- readRDS("WR/wr.kmat.rds")
wr.sf.95 <- readRDS("WR/wr.sf.95.rds")
...
wr.sf.126.2050 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(ndvi.ssp126, by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp126.runoff.2050)), by = "gid") %>% merge(ssp126.ndep.2050)

wr.sf.370.2050 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(ndvi.ssp370, by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp370.runoff.2050)), by = "gid") %>% merge(ssp370.ndep.2050)
...
wr.sf.126.2100 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(ndvi.ssp126.2100, by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp126.runoff.2100)), by = "gid") %>% merge(ssp126.ndep.2100)

wr.sf.370.2100 <- dplyr::select(wr.sf.95, c("gid","Bog","Runoff","logRunoff","Temp","Precip","NDVI","Arable","TNdep")) %>% setNames(c("gid","Bog","Runoff95","logRunoff95","Temp95","Prec95","NDVI95","Arable","TNdep95","geometry")) %>% merge(ndvi.ssp370.2100, by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp370.runoff.2100)), by = "gid") %>% merge(ssp370.ndep.2100)
...
n1 <- ggplot(wr.sf.126.2050)+geom_point(aes(x = NDVI95, y = NDVI))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
n2 <- ggplot(wr.sf.126.2100)+geom_point(aes(x = NDVI95, y = NDVI))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
n3 <- ggplot(wr.sf.370.2050)+geom_point(aes(x = NDVI95, y = NDVI))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
n4 <- ggplot(wr.sf.370.2100)+geom_point(aes(x = NDVI95, y = NDVI))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()

ggarrange(n1,n2,n3,n4, nrow = 2, ncol = 2)

t1 <- ggplot(wr.sf.126.2050)+geom_point(aes(x = Temp95, y = Temp))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
t2 <- ggplot(wr.sf.126.2100)+geom_point(aes(x = Temp95, y = Temp))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
t3 <- ggplot(wr.sf.370.2050)+geom_point(aes(x = Temp95, y = Temp))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
t4 <- ggplot(wr.sf.370.2100)+geom_point(aes(x = Temp95, y = Temp))+geom_abline(slope = 1, intercept = 0, col = "red")+theme_minimal()
...
```{r map-fit-toc-95-bis}
knitr::include_graphics("WR/wr.log.toc.png")
```

```{r plots-forecast-toc, eval = T, fig.dim= c(12,16)}
wr.ssp126.2050 <- readRDS("WR/wr.ssp126.2050.rds")
wr.ssp370.2050 <- readRDS("WR/wr.ssp370.2050.rds")
...
for(x in c("fit","diffTOC","diff.log","exp.diff.log")){
  all.vec <- c()
...
tt <- grid.arrange(t1, t2, t3, t4, ncol = 2, nrow = 2)
ggsave( "forecast.log.toc.png", tt)
```

```{r map-percent-change}
...
g <- ggplot(list.sf[["SSP370.2100"]])+
    geom_sf(aes(fill=diff.log, col= diff.log), size= 0.05)+
    scale_fill_continuous_divergingx(palette = "RdYlBu", aesthetics = c("fill","col"),name="d) Difference logTOC\n SSP3-7.0 - 2100", mid = 0, limits = c(min.lim[3],max.lim[3]), rev = T) +
    theme_minimal(base_size = 10)+theme(legend.position = "bottom")

ggsave(plot = g, filename = "WR/SSP370.2100.diff.log.png",dpi = "retina", width = 12, height = 16, units = "cm")
```

```{r ndvi-future-values}
list.sf$SSP126.2050$NDVI %>% boxplot

ggplot(subset(list.sf[["SSP126.2050"]], NDVI > 1))+
    geom_sf(aes(fill=NDVI, col= NDVI), size= 0.05)+
    scale_fill_continuous_divergingx(palette = "RdYlBu", aesthetics = c("fill","col"),name="a) Future NDVI > 1\n SSP1-2.6 - 2050", limits = c(1,1.4), rev = T) +
    theme_minimal(base_size = 10)+theme(legend.position = "bottom")

list.sf$SSP126.2100$NDVI %>% boxplot

ggplot(subset(list.sf[["SSP126.2100"]], NDVI > 1))+
    geom_sf(aes(fill=NDVI, col= NDVI), size= 0.05)+
    scale_fill_continuous_divergingx(palette = "RdYlBu", aesthetics = c("fill","col"),name="b) Future NDVI > 1\n SSP1-2.6 - 2100", limits = c(1,1.5), rev = T) +
    theme_minimal(base_size = 10)+theme(legend.position = "bottom")

list.sf$SSP370.2050$NDVI %>% boxplot

ggplot(subset(list.sf[["SSP370.2050"]], NDVI > 1))+
    geom_sf(aes(fill=NDVI, col= NDVI), size= 0.05)+
    scale_fill_continuous_divergingx(palette = "RdYlBu", aesthetics = c("fill","col"),name="c) Future NDVI > 1\n SSP3-7.0 - 2050", limits = c(1,1.5), rev = T) +
    theme_minimal(base_size = 10)+theme(legend.position = "bottom")

list.sf$SSP370.2100$NDVI %>% boxplot

ggplot(subset(list.sf[["SSP370.2100"]], NDVI > 1))+
    geom_sf(aes(fill=NDVI, col= NDVI), size= 0.05)+
    scale_fill_continuous_divergingx(palette = "RdYlBu", aesthetics = c("fill","col"),name="d) Future NDVI > 1\n SSP3-7.0 - 2100", limits = c(1,1.7), rev = T) +
    theme_minimal(base_size = 10)+theme(legend.position = "bottom")
...
ggplot() +
  ...
  geom_label(data = filter(toc.export.df, Model == "SSP126"), 
             aes(x=Year, y = TOC_Tg, col = Model, label = round(TOC_Tg,1)), nudge_y = -1.5, size = 2, label.padding = unit(0.2,"lines"), show_guide = F) +
  geom_label(data = filter(toc.export.df, Model == "SSP370"), 
             aes(x=Year, y = TOC_Tg, col = Model, label = round(TOC_Tg,1)), nudge_y = 1.5, size = 2, label.padding = unit(0.2,"lines"), show_guide = F) +
  geom_label(data = filter(toc.export.df, Model == "Historical"), 
             aes(x=Year, y = TOC_Tg, col = Model, label = round(TOC_Tg,1)), nudge_y = 1.5, size = 2, label.padding = unit(0.2,"lines"), show_guide = F) +
