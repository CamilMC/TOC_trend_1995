...
```{r comparare-datasets-plot, eval = F}
...
niva.near <- st_join(niva.sf, fennoscandia.sf, join = st_nearest_feature)

niva.poly <- merge(dplyr::select(fennoscandia.33,c("ebint","geom")), st_drop_geometry(niva.near), by = "ebint")
...
```{r compare-toc}
ggplot(niva.poly)+geom_point(aes(x=TOC,y=toc_2019))
```
...
saveRDS(runoff.niva,"WR/runoff.niva.rds")
...
list.clc <- lapply(clc.files,readRDS)

saveRDS(list.clc,"NIVA_test/CLC/list.clc.niva.rds")
```

```{r corine-tab-prop, eval = F}
list.clc.niva <- readRDS("NIVA_test/CLC/list.clc.niva.rds")
clc.tab <- lapply(list.clc.niva, function(x) tabulate(x,45))
...
runoff.niva <- readRDS("NIVA_test/runoff.niva.rds")
...
niva.data <- niva.poly %>% merge(ndvi.niva, by = "ebint") %>% merge(runoff.niva, by = "ebint") %>% merge(bogs.niva, by = "ebint") %>% merge(arable.niva, by = "ebint") %>% merge(ndep.df.niva, by = "ebint")
...
which(is.na(niva.data$toc_2019 == T))
