...
plot(catchment.poly.corine)
st_write(catchment.poly.corine, "Fennoscandia_NTNU/catchment.poly.shp")
...
year.ndvi <- raster::extract(year.mean,catchment.poly, fun = mean, na.rm = T, df = T, sp = T)
...
```{r sem-year, eval = F}
new.sem <- errorsarlm(fm,new.set,k.neigh.w.set)
...
# Modelling NDVI from T and precipitations
...
annual.temp <- raster::extract(bio.fennoscandia[[1]], head(catchment.poly), fun = mean, df = T, sp = T)
...
tmin.fennoscandia <- getData(name = "worldclim",var = "tmin",res=30)
tmin.summer <- raster::stack(tmin.fennoscandia[[6]],tmin.fennoscandia[[7]],tmin.fennoscandia[[8]]) %>% mean()
tmin.catch <- raster::extract(tmin.summer,catchment.poly,fun = mean, na.rm = T, df = T, sp = T)
saveRDS(tmin.catch,"tmin.catch.Rdata")

tmax.fennoscandia <- getData(name = "worldclim",var = "tmax",res=10)
tmax.summer <- raster::stack(tmax.fennoscandia[[6]],tmax.fennoscandia[[7]],tmax.fennoscandia[[8]]) %>% mean()
tmax.catch <- raster::extract(tmax.summer,catchment.poly,fun = mean, na.rm = T, df = T, sp = T)

prec.fennoscandia <- getData(name = "worldclim",var = "prec",res=10)
prec.summer <- raster::stack(prec.fennoscandia[[6]],prec.fennoscandia[[7]],prec.fennoscandia[[8]])
prec.catch <- raster::extract(prec.summer,catchment.poly,fun = mean, na.rm = T, df = T, sp = T)
...
clc.remote <- raster::extract(corine,head(catchment.poly.corine))
clc.tab <- sapply(clc.remote, function(x) tabulate(x,44))
