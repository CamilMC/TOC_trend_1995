...
* Mean runoff (Runoff) values as L/m2/s (i.e., mm/s) were derived from the CORDEX model for the period 1960 to 199031 @CORDEXvERC @Kreienkamp2012. As the runoff intensities were skewed the data were log-transformed (log Runoff) to normalize the distribution (`r fig_nums("insert-maps", display = "c")`c). 
...
knitr::include_graphics(c("NDVI/ndvi.fen.95.png", "Corine_Land_Cover/map-clc2.png"))
```
...
Once again, the effect size is expressed as % change of TOC concentration for a 1% increase in NDVI, Runoff and Bog coverage. These relative changes are arbitrarily chosen to represent the impact of each predictor variable on a comparable basis.
...
In 2019, a new national survey has been conducted over the 1000 Norwegian lakes @NIVA2020. The TOC concentration was measured in the selected samples. Catchment characteristics (land cover and runoff) were also computed for the needs of the survey. Therefore, we tested the SELM fitted in  to predict the TOC concentration based on NDVI, Runoff and Bog as predictor. We then compared the TOC computed by the model to the sampled values to assess the performance of our model. 

```{r fitted-vs-obs-niva}
knitr::include_graphics(c("NIVA_test/obs_niva.png","NIVA_test/fitted_niva.png","NIVA_test/diff.niva.png"))
```

`r fig_nums("niva_fit", "Observed, fitted and difference between observed and fitted values for TOC concentration on the 2019-survey data based on the SELM model with NDVI, Bog and log(Runoff) as predictors")`

The SELM predicts TOC concentration within a smaller range than the observed values, under-fitting the TOC concentration in the South-East of Norway (down to a difference of 10 mgC/L) and over-fitting the TOC value in the North and in altitude (`r fig_nums("niva_fit",display = "c")`. However, most predicted values are in the right range. Using Forest as predictor instead of NDVI results in the same pattern (`r fig_nums("niva_fit_forest",display = "c")`). 

```{r fitted-vs-obs-forest-niva}
knitr::include_graphics(c("NIVA_test/obs.niva.forest.png","NIVA_test/fitted.niva.forest.png","NIVA_test/diff.niva.forest.png"))
```

`r fig_nums("niva_fit_forest", "Observed, fitted and difference between observed and fitted values for TOC concentration on the 2019-survey data based on the SELM model with Forest, Bog and log(Runoff) as predictors")`

These results clearly show that the space-for-time approach does not take into account all the processes that explain the TOC concentration in lakes. The space-for-time approach uses a set of predictors taken at a single moment in time , but in a large variety of regions, to model future changes. However, the prediction range and the difference observed/predicted gives a good indication in most catchments. The discrepancy between fitted and observed values may be due catchment variables left out of the model, like thickness of soil (being tested now) or to long-term processes such as soil organic matter formation in the catchment.
...
The largest climate largest induced increases in DOM concentrations are predicted to mainly be in in the northern water regions and on the Norwegian west coast. A slight decrease in DOM is predicted for mid Sweden and Finland under the worse-case, though this is less distinct for the  2ºC scenario. The test on 2019-data showed that the values for these regions were over fitted, but the overfit is partially compensated by the fact that the 1995 values are also modelled based on the SELM model. THe decrease of TOC concentration seems to be linked by a decrease of both NDVI and runoff in the south-est region. On the contrary, the NDVI is going to increase mainly on the west and in the North, where runoff changes pattern are more contrasted. These are also the regions where the TOC concentration in lakes is expected to increase the most. 
...
However, the potential increase in the proportion of forest cover differs for all the water regions as the proportion of forest in a region with a 100% forest cover cannot increase further. Likewise, it is unlikely that a water region with a 0% forest cover (bare rocks, glaciers) can be afforested. Therefore, the potential increase of forest cover was calculated using a beta-distribution shape (see Supplementary 3). The increase was calculated to be, on average, 0,34% for Finland, 0.07% for Sweden and 14% for Norway.

```{r boxplot-forest-increase}
knitr::include_graphics(c("WR/boxplot.forest.png"))
```
...
knitr::include_graphics(c("WR/map.forest.png","WR/forest.increase.norge.png","WR/forest.increase.sweden.png","WR/forest.increase.finland.png" ))
```
...
* Runoff intensity (mm/year) is a major spatial predictor for TOC, with higher runoff intensity having a negative effect on TOC concentration (Figure 4). The dilution effect is therefore on average stronger than the mobilization of soil organic matter. 
...
* Forest model allows to assess the impact of afforestation. . The proportion of afforestation in Sweden and Finland is small and will lead to no visible difference. However, in Norway, more forest will result in  more TOC in lakes, up to +2 mg/L (Figure `r fig_nums("map-toc-forest", display = "c")`.
