...
wr.sf.95 <- readRDS("WR/wr.sf.95.rds")
ndvi.model <- errorsarlm(formula = ndvi~temp+log(prec), data = wr.sf.95,wr.kmat)
...
ndvi.ssp126.fit <- cbind(df.ssp126, ndvi.ssp126[,1]) %>% setNames(c("gid","temp126","prec126","ndvi126")) %>% sp::merge(select(wr.sf.95,c("gid","temp","prec","ndvi")), by = "gid") %>% st_as_sf()
...
ndvi.ssp370.fit <- cbind(df.ssp370, ndvi.ssp370[,1]) %>% setNames(c("gid","temp370","prec370","ndvi370")) %>% sp::merge(select(wr.sf.95,c("gid","temp","prec","ndvi")), by = "gid") %>% st_as_sf()
