...
```{r plot-runoff}
runoff.sf <- readRDS("runoff.sf.Rdata")
...
wr.df <- readRDS("WR/wr.df.Rdata")

ggplot()+geom_sf(data=wr.sf,aes(col = "bogs"))
...
saveRDS(wr.sf.2, "wr.sf.2.Rdata")
```
```{r plot-variables}
wr.sf.2 <- readRDS("WR/wr.sf.2.Rdata")

wr.bogs.plot <- ggplot()+geom_sf(data = wr.sf.2,aes(fill=bogs,col=bogs))+ scale_fill_gradient2(low="#CCCCCC",mid="#FFCC66",high = "#663300",midpoint = 0, aesthetics = c("fill","col"))+theme_void()
ggsave(plot = wr.bogs.plot , filename = "WR/wr.bogs.plot.png",dpi = "retina", width = 30, height = 90, units = "cm")

wr.ndvi.plot <- ggplot()+geom_sf(data = wr.sf.2,aes(fill=ndvi,col=ndvi))+ scale_fill_gradient2(low="#CCCCCC",mid="#FFCC66",high = "#003300",midpoint = 0, aesthetics = c("fill","col"))+theme_void()
ggsave(plot = wr.ndvi.plot , filename = "WR/wr.ndvi.plot.png",dpi = "retina", width = 30, height = 90, units = "cm")

wr.runoff.plot <- ggplot()+geom_sf(data = wr.sf.2,aes(fill=runoff,col=runoff))+ scale_fill_gradient2(low="#CCCCCC",mid="#CCFFFF",high = "#003399",midpoint = 0, aesthetics = c("fill","col"))+theme_void()
ggsave(plot = wr.runoff.plot , filename = "WR/wr.runoff.plot.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
sem.pred.wr <- cbind(wr.sf.2$gid,wr.pred) %>% as.data.frame() %>% setNames(c("gid","fit.log.toc"))
sem.pred.wr$fit.toc <- 10^sem.pred.wr$fit.log.toc

saveRDS(sem.pred.wr,"WR/sem.pred.wr.rds")

map.fit <- ggplot()+geom_sf(data = sem.pred.wr, aes(fill = fit.toc, col = fit.toc))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1)+theme_minimal()
ggsave(plot = map.fit, filename = "fit.toc.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.fit)

map.log <- ggplot()+geom_sf(data = sem.pred.wr, aes(fill = fit.log.toc, col = fit))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1)+theme_minimal()
ggsave(plot = map.log, filename = "fit.log.toc.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.log)
```

```{r check-outliers-toc, eval = F}
outliers <- ggplot()+geom_sf(data = filter(sem.pred.wr, fit.toc >15), aes(fill = fit.toc, col = fit.toc))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1)+theme_minimal()
outliers

boxplot(sem.pred.wr$fit.toc)
boxplot(sem.pred.wr$fit)

ggplot(sem.pred.wr)+geom_point(aes(x=size,y=fit))
ggplot(sem.pred.wr)+geom_point(aes(x=log.runoff,y=fit.toc))
ggplot(sem.pred.wr)+geom_point(aes(x=ndvi,y=fit.toc,col=size))
ggplot(sem.pred.wr)+geom_point(aes(x=bogs,y=fit))
ggplot(sem.pred.wr)+geom_point(aes(x=forest,y=fit))
...
temp.wr <- readRDS("temp.wr.rds")

#prec.wr <- raster::extract(bio.fennoscandia$bio12, wr.sf.2, fun = mean, df = T, sp = T)
#saveRDS(prec.wr,"prec.wr.rds")
prec.wr <- readRDS("prec.wr.rds")

wr.sf.3 <- wr.sf.2
wr.sf.3$temp <- temp.wr$bio1
wr.sf.3$prec <- prec.wr$bio12
saveRDS(wr.sf.3,"wr.sf.3.rds")

wr.sf.3 <- readRDS("wr.sf.3.rds")

wr.kmat <- readRDS("wr.kmat.Rdata")
ndvi.model <- errorsarlm(formula = ndvi~temp+log10(prec), data = wr.sf.3,wr.kmat)
saveRDS(ndvi.model, "ndvi.model.rds")
...
ssp126.temp.wr <- raster::extract(ssp126.cnrm.temp,select(wr.sf.3,c("gid","geometry")),fun=mean,df = T, sp = T)
names(ssp126.temp.wr) <- c("gid", "temp126")
saveRDS(ssp126.temp.wr,"WR/ssp126.temp.wr.rds")

ssp370.temp.wr <- raster::extract(ssp370.cnrm.temp,select(wr.sf.3,c("gid","geometry")),fun=mean,df = T, sp = T)
...
ssp126.prec.wr <- raster::extract(ssp126.cnrm.prec,select(wr.sf.3,c("gid","geometry")),fun=mean,df = T, sp = T)
names(ssp126.prec.wr) <- c("gid", "prec126")
saveRDS(ssp126.prec.wr,"WR/ssp126.prec.wr.rds")

ssp370.prec.wr <- raster::extract(ssp370.cnrm.prec,dplyr::select(wr.sf.3,c("gid","geometry")),fun=mean,df = T, sp = T)

names(ssp370.prec.wr) <- c("gid", "prec370")
saveRDS(ssp370.prec.wr,"WR/ssp370.prec.wr.rds")

ssp126.temp.plot <- ggplot()+geom_sf(data=st_as_sf(ssp126.temp.wr),aes(fill=temp126))+scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#990000",midpoint = 0)
ggsave(plot = ssp126.temp.plot,filename = "WR/ssp126.temp.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.temp.plot <- ggplot()+geom_sf(data=st_as_sf(ssp370.temp.wr),aes(fill=temp370))+scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#990000",midpoint = 0)
ggsave(plot = ssp370.temp.plot,filename = "WR/ssp370.temp.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp126.prec.plot <- ggplot()+geom_sf(data=st_as_sf(ssp126.prec.wr),aes(fill=prec126))+scale_fill_gradient2(low="#EEE9E9",mid="#CCCFFF",high = "#0033FF",midpoint = 0)
ggsave(plot = ssp126.prec.plot,filename = "WR/ssp126.prec.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.prec.plot <- ggplot()+geom_sf(data=st_as_sf(ssp370.prec.wr),aes(fill=prec370))+scale_fill_gradient2(low="#EEE9E9",mid="#CCCFFF",high = "#0033FF",midpoint = 0)
ggsave(plot = ssp370.prec.plot,filename = "WR/ssp370.prec.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
df.ssp126.nona <- df.ssp126[-which(is.na(df.ssp126$temp) == T),]
ggplot()+geom_sf(data=st_as_sf(df.ssp126[which(is.na(df.ssp126$temp) == T),]), aes(fill = gid))
missing.gid <- df.ssp126$gid[which(is.na(df.ssp126$temp) == T)]

missing.temp126 <- raster::extract(ssp126.cnrm.temp,wr.sf.3[which(wr.sf.3$gid %in% missing.gid),],fun=mean,df = T, sp = T)
names(missing.temp126) <- c("gid", "temp126")
saveRDS(missing.temp126,"WR/missing.temp126.rds")
ggplot(st_as_sf(missing.temp126))+geom_sf(aes(fill=temp126))

missing.prec126 <- raster::extract(ssp126.cnrm.prec,wr.sf.3[which(wr.sf.3$gid %in% missing.gid),],fun=mean,df = T, sp = T)
names(missing.temp126) <- c("gid", "prec126")
saveRDS(missing.prec126,"WR/missing.prec126.rds")
ggplot(st_as_sf(missing.prec126))+geom_sf(aes(fill=prec126))

ndvi.ssp126 <- predict(object = ndvi.model, newdata = df.ssp126.nona,listw = wr.kmat, pred.type = "TS", legacy.mixed = F, zero.policy = T)

df.ssp126.fit <- cbind(df.ssp126.nona, ndvi.ssp126) %>% setNames(c("gid","temp","prec","fit")) %>% sp::merge(wr.sf.3[,1:2], by = "gid") %>% st_as_sf()

df.ssp126.fit$ndvi.diff <- df.ssp126.fit$fit-df.ssp126.fit$ndvi
df.ssp126.fit$percent.change <- df.ssp126.fit$ndvi.diff/df.ssp126.fit$ndvi

saveRDS(df.ssp126.fit,"WR/ndvi.ssp126.fit.rds")

df.ssp370 <- merge(ssp370.temp.wr, ssp370.prec.wr, by = "gid") %>% setNames(c("gid","temp","prec"))
df.ssp370.nona <- df.ssp370[-which(is.na(df.ssp370$temp) == T),]

missing.gid370 <- df.ssp370$gid[which(is.na(df.ssp370$temp) == T)]
missing.temp370 <- raster::extract(ssp370.cnrm.temp,wr.sf.3[which(wr.sf.3$gid %in% missing.gid),],fun=mean,df = T, sp = T)
names(missing.temp370) <- c("gid", "temp370")
saveRDS(missing.temp370,"WR/missing.temp370.rds")
ggplot(st_as_sf(missing.temp126))+geom_sf(aes(fill=temp126))
...
ndvi.ssp370 <- predict(object = ndvi.model, newdata = df.ssp370.nona, listw = wr.kmat, pred.type = "TS", legacy.mixed = F, zero.policy = T)

df.ssp370.fit <- cbind(df.ssp370.nona, ndvi.ssp370) %>% setNames(c("gid","temp","prec","fit")) %>% sp::merge(wr.sf.3[,1:2], by = "gid") %>% st_as_sf()

df.ssp370.fit$ndvi.diff <- df.ssp370.fit$fit-df.ssp370.fit$ndvi
df.ssp370.fit$percent.change <- df.ssp370.fit$ndvi.diff/df.ssp370.fit$ndvi

saveRDS(df.ssp370.fit,"WR/ndvi.ssp370.fit.rds")
```

```{r plot-future-ndvi}
ssp126.diff.ndvi <- ggplot()+geom_sf(data=df.ssp126.fit,aes(fill=ndvi.diff))+ scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0)
ggsave(plot = ssp126.diff.ndvi,filename = "WR/ssp126.ndvi.diff.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp126.percent.ndvi <- ggplot()+geom_sf(data=df.ssp126.fit,aes(fill=percent.change))+ scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0)
ggsave(plot = ssp126.percent.ndvi,filename = "WR/ssp126.percent.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.diff.ndvi <- ggplot()+geom_sf(data=df.ssp370.fit,aes(fill=ndvi.diff))+    scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0)
ggsave(plot = ssp370.diff.ndvi,filename = "WR/ssp370.ndvi.diff.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.percent.ndvi <- ggplot()+geom_sf(data=df.ssp370.fit,aes(fill=percent.change)) + scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0)
ggsave(plot = ssp370.percent.ndvi,filename = "WR/ssp370.precent.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")

```

```{r extract-future-runoff}
ssp126.runoff.wr <- raster::stack("CORDEX_future/mrros_Lmon_CNRM-CM6-1-HR_ssp126_r1i1p1f2_gr_201501-210012.nc", bands = c(421:432))
ssp126.runoff.wr.mean <- ssp126.runoff.wr %>% mean()

runoff.ssp126 <- raster::extract(ssp126.runoff.wr.mean,dplyr::select(wr.sf.3,c("gid","geometry")),fun=mean,df = T, sp = T)
names(runoff.ssp126) <- c("gid","runoff126")
saveRDS(runoff.ssp126,"WR/ssp126.runoff.rds")

ssp370.runoff.wr <- raster::stack("CORDEX_future/mrros_Lmon_CNRM-CM6-1-HR_ssp370_r1i1p1f2_gr_201501-210012.nc", bands = c(421:432))
...
ssp126.runoff.plot <- ggplot()+geom_sf(data = st_as_sf(ssp126.runoff.wr),aes(fill=runoff126,col=runoff126))+ scale_fill_gradient2(low="#CCCCCC",mid="#CCFFFF",high = "#003399",midpoint = 0, aesthetics = c("fill","col"))+theme_void()
ggsave(plot = ssp126.runoff.plot , filename = "WR/ssp126.runoff.plot.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.runoff.plot <- ggplot()+geom_sf(data = st_as_sf(ssp370.runoff.wr),aes(fill=runoff370,col=runoff370))+ scale_fill_gradient2(low="#CCCCCC",mid="#CCFFFF",high = "#003399",midpoint = 0, aesthetics = c("fill","col"))+theme_void()
ggsave(plot = ssp370.runoff.plot , filename = "WR/ssp370.runoff.plot.png",dpi = "retina", width = 30, height = 90, units = "cm")
```

```{r future-wr}
ndvi.ssp126.fit <- readRDS("WR/ndvi.ssp126.fit.rds") %>% as.data.frame() %>% dplyr::select(c("gid","fit","ndvi.diff","percent.change")) %>% setNames(c("gid","ndvi126","ndvi.diff126", "percent.change126"))
ndvi.ssp370.fit <- readRDS("WR/ndvi.ssp370.fit.rds") %>% as.data.frame() %>% dplyr::select(c("gid","fit","ndvi.diff","percent.change")) %>% setNames(c("gid","ndvi370","ndvi.diff370", "percent.change370"))



wr.sf.4 <- merge(wr.sf.3, sem.pred.wr) %>% merge(ssp126.temp.wr) %>% merge(ssp370.temp.wr) %>% merge(ssp126.prec.wr) %>% merge(ssp370.prec.wr) %>% merge(ssp126.runoff) %>% merge(ssp370.runoff) %>% merge(ndvi.ssp126.fit) %>% merge(ndvi.ssp370.fit) 

na.lines <- which(complete.cases(st_drop_geometry(wr.sf.4)) == F)
ggplot()+geom_sf(data=wr.sf.4[na.lines,],aes(fill=gid))
ggplot()+geom_sf(data = wr.sf.4[which(is.na(wr.sf.4$bogs)==T),], aes(fill = gid))

wr.sf.4 <- wr.sf.4[-na.lines,]
wr.sf.4$runoff126 <- wr.sf.4$runoff126*365*24*60*60 # from kg/m2/s to kg/m2/year
wr.sf.4$runoff370 <- wr.sf.4$runoff370*365*24*60*60 # from kg/m2/s to kg/m2/year

wr.sf.4$percent.runoff126 <- wr.sf.4$runoff126/wr.sf.4$runoff * 100
wr.sf.4$percent.runoff370 <- wr.sf.4$runoff370/wr.sf.4$runoff * 100
...
ggsave(plot = map.ndvi, filename = "WR/effect.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
ggsave(plot = map.ndvi, filename = "WR/effect.ndvi.370.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
ggsave(plot = map.runoff, filename = "WR/effect.runoff.126.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
ggsave(plot = map.runoff, filename = "WR/effect.runoff.370.png",dpi = "retina", width = 30, height = 90, units = "cm")
```
```{r sem-model-toc-wr-2050}
library(spatialreg)
sem.fen.simple <- readRDS("sem.fen.simple.Rdata")
fm <- log.toc~ndvi+log.runoff+bogs
newdata.ssp126 <- wr.sf.4 %>% dplyr::select(c("gid","bogs","runoff126","ndvi126")) %>% setNames(c("gid","bogs","runoff","ndvi","ndvi.percent","geometry"))
newdata.ssp126$log.runoff <- log10(newdata.ssp126$runoff)

wr.pred.ssp126 <- predict(sem.fen.simple,wr.sf.2,wr.kmat, pred.type = "TS",legacy.mixed = F, zero.policy = T)

result <- cbind(wr.sf.2,wr.pred)
result$fit.toc <- 10^result$fit
map.fit <- ggplot()+geom_sf(data = result, aes(fill = fit.toc, col = fit.toc))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1)+theme_minimal()
ggsave(plot = map.fit, filename = "fit.toc.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.fit)

map.log <- ggplot()+geom_sf(data = result, aes(fill = fit, col = fit))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1)+theme_minimal()
ggsave(plot = map.log, filename = "fit.log.toc.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.log)
```
