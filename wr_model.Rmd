...
outliers <- ggplot()+geom_sf(data = filter(result, fit.toc >15), aes(fill = fit.toc, col = fit.toc))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1)+theme_minimal()
outliers

boxplot(result$fit.toc)
boxplot(result$fit)

ggplot(result)+geom_point(aes(x=size,y=fit))
ggplot(result)+geom_point(aes(x=log.runoff,y=fit.toc))
ggplot(result)+geom_point(aes(x=ndvi,y=fit.toc,col=size))
ggplot(result)+geom_point(aes(x=bogs,y=fit))
ggplot(result)+geom_point(aes(x=forest,y=fit))
...
ndvi.model <- errorsarlm(formula = ndvi~temp+prec, data = wr.sf.3,wr.kmat)
...
saveRDS(ssp126.temp.wr,"ssp126.temp.wr.rds")
...
saveRDS(ssp126.prec.wr,"ssp126.prec.wr.rds")

ssp370.prec.wr <- raster::extract(ssp370.cnrm.prec,select(wr.sf.3,c("gid","geometry")),fun=mean,df = T, sp = T)
names(ssp370.prec.wr) <- c("gid", "prec370")
saveRDS(ssp370.prec.wr,"ssp370.prec.wr.rds")


boxplot(ssp126.temp.wr$temp126)
boxplot(ssp370.temp.wr$temp370)
boxplot(ssp126.prec.wr$prec126)
boxplot(ssp370.prec.wr$prec370)

ndvi.model <- readRDS("ndvi.model.rds")
df.ssp126 <- merge(ssp126.temp.wr, ssp126.prec.wr,by = "gid")%>% setNames(c("gid","temp","prec"))
ndvi.ssp126 <- predict(ndvi.model,df.ssp126,wr.kmat, pred.type = "TS",legacy.mixed = F, zero.policy = T)
...
result$effect.bogs <- result$fit.toc + result$fit.toc*(10^(sem.coef[3,1]*0.1))
result$diff.effect.ndvi <-  result$fit.toc*(10^(sem.coef[1,1]*0.1))
result$diff.effect.runoff <-  result$fit.toc*(1.1^(sem.coef[2,1]))
result$diff.effect.bogs <- result$fit.toc*(10^(sem.coef[3,1]*0.1))

legend.limit <- c(min(result$fit.toc), max(c(result$effect.ndvi,result$effect.runoff,result$effect.bogs)))

map.ndvi <- ggplot()+geom_sf(data = result, aes(fill = effect.ndvi, col = effect.ndvi))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, limits = legend.limit)+theme_minimal()
ggsave(plot = map.ndvi, filename = "effect.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.ndvi)

map.runoff <- ggplot()+geom_sf(data = result, aes(fill = effect.runoff, col = effect.runoff))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1,limits = legend.limit)+theme_minimal()
ggsave(plot = map.runoff, filename = "effect.runoff.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.runoff)

map.bogs <- ggplot()+geom_sf(data = result, aes(fill = effect.bogs, col = effect.bogs))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, limits = legend.limit)+theme_minimal()
ggsave(plot = map.bogs, filename = "effect.bogs.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.bogs)

legend.limit.diff <- c(min(c(result$effect.bogs,result$effect.ndvi,result$effect.runoff)), max(c(result$effect.bogs,result$effect.ndvi,result$effect.runoff)))
map.diff.ndvi <- ggplot()+geom_sf(data = result, aes(fill = diff.effect.ndvi, col = diff.effect.ndvi))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, limits = legend.limit.diff)+theme_minimal()
ggsave(plot = map.ndvi, filename = "diff.effect.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.diff.ndvi)

map.diff.runoff <- ggplot()+geom_sf(data = result, aes(fill = diff.effect.runoff, col = diff.effect.runoff))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, limits = legend.limit.diff)+theme_minimal()
ggsave(plot = map.runoff, filename = "diff.effect.runoff.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.diff.runoff)

map.diff.bogs <- ggplot()+geom_sf(data = result, aes(fill = diff.effect.bogs, col = diff.effect.bogs))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, limits = legend.limit.diff)+theme_minimal()
ggsave(plot = map.bogs, filename = "diff.effect.bogs.png",dpi = "retina", width = 30, height = 90, units = "cm")
print(map.diff.bogs)
```
