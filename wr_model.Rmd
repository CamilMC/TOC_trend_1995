...
all.wr.sf <- merge(wr.sf,runoff.wr95,by = "gid") %>% merge(ndvi.wr,by="gid") %>% merge(clc.wr.sum,by = "gid") %>% merge(st_drop_geometry(tp),by = "gid")
...
wr.sf.95 <- all.wr.sf %>% filter(ndvi != 0) %>% filter(runoff > 0) %>% filter(is.na(bogs) == F)
...
ndvi.model <- lm(formula = ndvi~temp+log(prec), data = wr.sf.95)
...
ndvi.ssp126.fit <- cbind(df.ssp126, ndvi.ssp126) %>% setNames(c("gid","temp126","prec126","ndvi126")) %>% sp::merge(select(wr.sf.95,c("gid","temp","prec","ndvi")), by = "gid") %>% st_as_sf()
...
ndvi.ssp370.fit <- cbind(df.ssp370, ndvi.ssp370) %>% setNames(c("gid","temp370","prec370","ndvi370")) %>% sp::merge(select(wr.sf.95,c("gid","temp","prec","ndvi")), by = "gid") %>% st_as_sf()
...
ssp126.diff.ndvi <- ggplot()+geom_sf(data=ndvi.ssp126.fit,aes(fill=ndvi126-ndvi, col = ndvi126-ndvi)) + 
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0, aesthetics = c("fill","col"),
                       name = "Difference NDVI 1995-2050\n under SSP126", limits = limits.ndvi) + 
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
ssp126.ndvi <- ggplot()+geom_sf(data=ndvi.ssp126.fit,aes(fill=ndvi, col = ndvi)) + 
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0, aesthetics = c("fill","col"),
                       name = "NDVI 2050 under SSP126", limits = limits.diff.ndvi) + 
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
ssp370.diff.ndvi <- ggplot()+geom_sf(data=ndvi.ssp370.fit,aes(fill=ndvi370-ndvi, col = ndvi370-ndvi))+ 
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0, aesthetics = c("fill","col"),
                       name = "Difference NDVI 1995-2050\n under SSP370", limits = limits.ndvi) + 
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
ssp370.ndvi <- ggplot()+geom_sf(data=ndvi.ssp370.fit,aes(fill=ndvi,col=ndvi)) + 
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0, aesthetics = c("fill","col"),
                       name = "NDVI 2050 under SSP370", limits = limits.diff.ndvi) + 
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
In the historical dataset in Worldclim, the runoff is given as a yearly average in kg/m2/s, or mm/s. By convenience we convert mm/s to mm/y by multiplying by 365*24*60*60 = . 
...
ssp126.runoff.diff <- ggplot()+geom_sf(data = runoff.126,aes(fill=runoff126-runoff,col=runoff126-runoff))+ 
  scale_fill_gradient2(low="#CCCCCC",mid="#CCFFFF",high = "#003399",midpoint = 0, aesthetics = c("fill","col"), name = "Difference runoff 1995-2050\n under SSP126 in mm/s", limits = limits.diff.runoff)+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
compare.ssp <- data.frame(value = c(temperature, precipitation, ndvi, runoff,toc), variable = c(rep("Temperature", length(temperature)), rep("Precipitation", length(precipitation)), rep("NDVI",length(ndvi)), rep("Runoff",length(runoff)), rep("TOC", length(toc))), model = rep(model,5)) 
...
map.toc.126 <- ggplot()+geom_sf(data = wr.ssp126, aes(fill = toc, col = toc))+
  scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, 
                       name = "Modelled TOC in 2050 under SSP126 (mg/L)", limits = limits.toc)+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
map.log.toc.126 <- ggplot()+geom_sf(data = wr.ssp126, aes(fill = fit, col = fit))+
  scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1,
                       name = "Modelled log(TOC) in 2050 under SSP126", limits = limits.log.toc)+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
map.toc.370 <- ggplot()+geom_sf(data = wr.ssp370, aes(fill = toc, col = toc))+
  scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1,
                       name = "Modelled TOC in 2050 under SSP370 (mg/L)", limits = limits.toc)+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
map.log.toc.370 <- ggplot()+geom_sf(data = wr.ssp370, aes(fill = fit, col = fit))+
  scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1,
                       name = "Modelled log(TOC) in 2050 under SSP370", limits = limits.log.toc)+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
ggsave(plot = map.log.toc.370, filename = "WR/map.log.toc.370.png",dpi = "retina", width = 6, height = 8, units = "cm")
 ...
 map.toc.diff.370 <- ggplot()+geom_sf(data = wr.ssp370, aes(fill = toc-toc95, col = toc-toc95))+
   scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1,
                        name = "Difference TOC concentration 1995-2050\n under SSP370", limits = limits.diff.toc)+
   theme_minimal(base_size = 6)+
   theme(legend.position = "bottom")
...
wr.forest <- merge(wr.forest.pred, select(st_drop_geometry(sem.pred.wr),c("gid","wr.pred.fit","toc95")))

map.toc.forest <- ggplot()+geom_sf(data = wr.forest, aes(fill = toc, col = toc))+
  scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, 
...
map.log.toc.forest <- ggplot()+geom_sf(data = wr.forest, aes(fill = fit, col = fit))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, name = "Modelled log(TOC) with increase of forest cover")+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
ggsave(plot = map.log.toc.forest, filename = "WR/map.log.toc.forest.png",dpi = "retina", width = 6, height = 8, units = "cm")

map.diff.toc.forest <- ggplot()+geom_sf(data = wr.forest, aes(fill = toc-toc95, col = toc-toc95))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, name = "Difference of TOC concentration\n with increase of forest cover")+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
