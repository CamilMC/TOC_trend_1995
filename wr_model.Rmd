...
sem.pred.wr$fit.toc <- 10^sem.pred.wr$wr.pred.fit
...
ggsave(plot = map.fit, filename = "WR/wr.toc.png",dpi = "retina", width = 30, height = 90, units = "cm")

map.log <- ggplot()+geom_sf(data = sem.pred.wr, aes(fill = wr.pred.fit, col = wr.pred.fit))+
  scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, 
                       name = "Modelled log(TOC)")+theme_minimal()
ggsave(plot = map.log, filename = "WR/wr.log.toc.png",dpi = "retina", width = 30, height = 90, units = "cm")
```
...
#missing.temp126 <- raster::extract(ssp126.cnrm.temp,dplyr::select(wr.sf.3[which(wr.sf.3$gid %in% missing.gid),],gid),fun=mean,df = T, sp = T, na.rm = T)
#names(missing.temp126) <- c("gid", "temp")
#saveRDS(missing.temp126,"WR/missing.temp126.rds")
missing.temp126 <- readRDS("WR/missing.temp126.rds")

#missing.prec126 <- raster::extract(ssp126.cnrm.prec,dplyr::select(wr.sf.3[which(wr.sf.3$gid %in% missing.gid),],gid),fun=mean,df = T, sp = T, na.rm = T)
#names(missing.prec126) <- c("gid", "prec")
#saveRDS(missing.prec126,"WR/missing.prec126.rds")
missing.prec126 <- readRDS("WR/missing.prec126.rds")


missing.126 <- merge(missing.temp126, missing.prec126)
df.ssp126.missing <- rbind(df.ssp126.nona,missing.126)
na.final <- which(is.na(df.ssp126.missing$temp) == T)
df.ssp126.final <- df.ssp126.missing[-which(is.na(df.ssp126.missing$temp) == T),]
saveRDS(df.ssp126.final, "WR/df.ssp126.final.rds")
...
# missing.temp370 <- raster::extract(ssp370.cnrm.temp,dplyr::select(wr.sf.3[which(wr.sf.3$gid %in% missing.gid370),],gid),fun=mean,df = T, sp = T, na.rm = T)
# names(missing.temp370) <- c("gid", "temp")
# saveRDS(missing.temp370,"WR/missing.temp370.rds")
missing.temp370 <- readRDS("WR/missing.temp370.rds")

# missing.prec370 <- raster::extract(ssp370.cnrm.prec,dplyr::select(wr.sf.3[which(wr.sf.3$gid %in% missing.gid370),],gid),fun=mean,df = T, sp = T, na.rm = T)
# names(missing.prec370) <- c("gid", "prec")
# saveRDS(missing.prec370,"WR/missing.prec370.rds")
missing.prec370 <- readRDS("WR/missing.prec370.rds")

missing.370 <- merge(missing.temp370, missing.prec370)
df.ssp370.missing <- rbind(df.ssp370.nona,missing.370)
na.final <- which(is.na(df.ssp370.missing$temp) == T)
df.ssp370.final <- df.ssp370.missing[-which(is.na(df.ssp370.missing$temp) == T),]
saveRDS(df.ssp370.final,"df.ssp370.final.rds")

```

```{r model-future-ndvi}
df.ssp126.final <- readRDS("df.ssp126.final.rds")
df.ssp370.final <- readRDS("df.ssp126.final.rds")

ndvi.ssp126 <- predict(object = ndvi.model, newdata = df.ssp126.final,listw = wr.kmat, pred.type = "TS", legacy.mixed = F, zero.policy = T)

ndvi.ssp126.fit <- cbind(df.ssp126.final, ndvi.ssp126[,1]) %>% setNames(c("gid","temp","prec","fit")) %>% sp::merge(wr.sf.3[,1:2], by = "gid") %>% st_as_sf()
...
ndvi.ssp370 <- predict(object = ndvi.model, newdata = df.ssp370.final, listw = wr.kmat, pred.type = "TS", legacy.mixed = F, zero.policy = T)

ndvi.ssp370.fit <- cbind(df.ssp370.final, ndvi.ssp370[,1]) %>% setNames(c("gid","temp","prec","fit")) %>% sp::merge(wr.sf.3[,1:2], by = "gid") %>% st_as_sf()
...
ssp126.temp.plot <- ggplot()+geom_sf(data=st_as_sf(ndvi.ssp126.fit),aes(fill= temp , col = temp))+
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#990000",midpoint = 0, aesthetics = c("fill","col"),
                       name = "2041-2060 average temperature under SSP126 (°C)") + 
  theme_minimal(base_size = 15) +
  theme(legend.position = "bottom", legend.text = element_text(size = 8)) 
...
ssp370.temp.plot <- ggplot()+geom_sf(data=st_as_sf(ndvi.ssp370.fit),aes(fill= temp , col = temp))+
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#990000",midpoint = 0, aesthetics = c("fill","col"), 
                       name = "2041-2060 average temperature under SSP370 (°C)") +
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
...
ssp126.prec.plot <- ggplot()+geom_sf(data=st_as_sf(ndvi.ssp126.fit),aes(fill=prec, col = prec))+
  scale_fill_gradient2(low="#EEE9E9",mid="#CAE1FF",high = "#00688B",midpoint = 0, aesthetics = c("fill","col"), 
                       name = "2041-2060 average precipitation under SSP126 (mm/year)") + 
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
...
ssp370.prec.plot <- ggplot()+geom_sf(data=st_as_sf(ndvi.ssp370.fit),aes(fill=prec, col = prec))+
  scale_fill_gradient2(low="#EEE9E9",mid="#CAE1FF",high = "#00688B",midpoint = 0, aesthetics = c("fill","col"), 
                       name = "2041-2060 average precipitation under SSP370 (mm/year)") +
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
...
ssp126.diff.ndvi <- ggplot()+geom_sf(data=ndvi.ssp126.fit,aes(fill=ndvi.diff, col = ndvi.diff)) + 
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0, aesthetics = c("fill","col"),
                       name = "Difference NDVI 1995-2005 under SSP126") + 
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
...
ssp126.ndvi <- ggplot()+geom_sf(data=ndvi.ssp126.fit,aes(fill=ndvi, col = ndvi)) + 
  ...
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
ggsave(plot = ssp126.ndvi,filename = "WR/ssp126.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.diff.ndvi <- ggplot()+geom_sf(data=ndvi.ssp370.fit,aes(fill=ndvi.diff, col = ndvi.diff))+ 
  scale_fill_gradient2(low="#EEE9E9",mid="#FFFFBF",high = "#008B45",midpoint = 0, aesthetics = c("fill","col"),
                       name = "Difference NDVI 1995-2050 under SSP370") + 
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
...
ssp370.ndvi <- ggplot()+geom_sf(data=ndvi.ssp370.fit,aes(fill=ndvi,col=ndvi)) + 
  ...
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
ggsave(plot = ssp370.ndvi,filename = "WR/ssp370.ndvi.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
ssp126.runoff.plot <- ggplot()+geom_sf(data = st_as_sf(ssp126.runoff),aes(fill=runoff126,col=runoff126))+ 
  scale_fill_gradient2(low="#CCCCCC",mid="#CCFFFF",high = "#003399",midpoint = 0, aesthetics = c("fill","col"), name = "Average runoff 2041-2060 under SSP126 in mm/year")+
  theme_minimal(base_size = 15)+theme(legend.position = "bottom", legend.text = element_text(size =8))
ggsave(plot = ssp126.runoff.plot , filename = "WR/ssp126.runoff.png",dpi = "retina", width = 30, height = 90, units = "cm")

ssp370.runoff.plot <- ggplot()+geom_sf(data = st_as_sf(ssp370.runoff),aes(fill=runoff370,col=runoff370))+ 
  scale_fill_gradient2(low="#CCCCCC",mid="#CCFFFF",high = "#003399",midpoint = 0, aesthetics = c("fill","col"), name = "Average runoff 2041-2060 under SSP370 in mm/year")+
  theme_minimal(base_size = 15)+theme(legend.position = "bottom", legend.text = element_text(size =8))
ggsave(plot = ssp370.runoff.plot , filename = "WR/ssp370.runoff.png",dpi = "retina", width = 30, height = 90, units = "cm")
```
...
#wr.sf.4 <- merge(wr.sf.3, sem.pred.wr) %>% merge(st_drop_geometry(ndvi.ssp126.fit), by = "gid")  %>% merge(st_drop_geometry(st_as_sf(ssp126.runoff)), by = "gid") %>% merge(st_drop_geometry(ndvi.ssp370.fit), by = "gid") %>% merge(st_drop_geometry(st_as_sf(ssp370.runoff)), by = "gid")
...
wr.ssp126$toc <- 10^wr.ssp126$fit
...
map.toc.126 <- ggplot()+geom_sf(data = wr.ssp126, aes(fill = toc, col = toc))+
  ...
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
ggsave(plot = map.toc.126, filename = "WR/map.toc.126.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
map.log.toc.126 <- ggplot()+geom_sf(data = wr.ssp126, aes(fill = fit, col = fit))+
  ...
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
ggsave(plot = map.log.toc.126, filename = "WR/map.log.toc.126.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
wr.ssp370$toc <- 10^wr.ssp370$fit
...
map.toc.370 <- ggplot()+geom_sf(data = wr.ssp370, aes(fill = toc, col = toc))+
  ...
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
ggsave(plot = map.toc.370, filename = "WR/map.toc.370.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
map.log.toc.370 <- ggplot()+geom_sf(data = wr.ssp370, aes(fill = fit, col = fit))+
  ...
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom", legend.text = element_text(size = 8))
ggsave(plot = map.log.toc.370, filename = "WR/map.log.toc.370.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
wr.forest.pred$toc <- 10^wr.forest.pred$pred.forest.fit
...
ggsave(plot = map.toc.forest, filename = "WR/map.toc.forest.png",dpi = "retina", width = 30, height = 90, units = "cm")
...
map.log.toc.forest <- ggplot()+geom_sf(data = wr.forest.pred, aes(fill = pred.forest.fit, col = pred.forest.fit))+scale_fill_distiller(type = "seq", palette = 8, aesthetics = c("colour","fill"), direction = 1, name = "Modelled log(TOC) with increase of forest cover")+
  theme_minimal(base_size = 15)+
  theme(legend.position = "bottom",legend.text = element_text(size = 8))
ggsave(plot = map.log.toc.forest, filename = "WR/map.log.toc.forest.png",dpi = "retina", width = 30, height = 90, units = "cm")
