...
# SSP126
df.ssp126 <- dplyr::select(wr.sf.95, c("gid","Alt","Slope","Latitude")) %>% merge(ssp126.temp.wr, by = "gid") %>% merge(ssp126.prec.wr, by = "gid") %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","geometry")) 
saveRDS(df.ssp126, "WR/df.ssp126.rds")

# SSP 370
df.ssp370 <- dplyr::select(wr.sf.95, c("gid","Alt","Slope","Latitude")) %>% merge(ssp370.temp.wr, by = "gid") %>% merge(ssp370.prec.wr, by = "gid") %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","geometry")) 
saveRDS(df.ssp370, "WR/df.ssp370.rds")

# SSP126 2100
df.ssp126.2100 <- dplyr::select(wr.sf.95, c("gid","Alt","Slope","Latitude")) %>% merge(ssp126.temp.2100, by = "gid") %>% merge(ssp126.prec.2100, by = "gid") %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","geometry")) 
saveRDS(df.ssp126.2100, "WR/df.ssp126.2100.rds")

# SSP 370 2100
df.ssp370.2100 <- dplyr::select(wr.sf.95, c("gid","Alt","Slope","Latitude")) %>% merge(ssp370.temp.2100, by = "gid") %>% merge(ssp370.prec.2100, by = "gid") %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","geometry")) 
...
ndvi.model <- errorsarlm(formula = NDVI~Temp+log(Slope)+log(Precip), data = wr.sf.95, listw = wr.kmat)
...
ndvi.ssp126 <- cbind(df.ssp126, ndvi.ssp126.fit) %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","NDVI","trend","signal","geometry"))
...
ndvi.ssp370 <- cbind(df.ssp370, ndvi.ssp370.fit) %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","NDVI","trend","signal","geometry"))
...
ndvi.ssp126.2100 <- cbind(df.ssp126.2100, ndvi.ssp126.2100.fit) %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","NDVI","trend","signal","geometry"))
...
ndvi.ssp370.2100 <- cbind(df.ssp370.2100, ndvi.ssp370.2100.fit) %>% setNames(c("gid","Alt","Slope","Latitude","Temp","Precip","NDVI","trend","signal","geometry"))
...
ssp126.runoff$Runoff126 <- ssp126.runoff$runoff126 *365*24*60*60
ssp370.runoff$Runoff370 <- ssp370.runoff$runoff370 *365*24*60*60



runoff.126 <- merge(select(wr.sf.95,c("gid","Runoff","logRunoff")),ssp126.runoff, by = "gid")
runoff.370 <- merge(select(wr.sf.95,c("gid","Runoff","logRunoff")),ssp370.runoff, by = "gid")
...
ssp370.runoff.diff <- ggplot()+geom_sf(data = runoff.370,aes(fill=Runoff370-Runoff,col=Runoff370-Runoff))+ 
  scale_fill_gradient2(low="#B03060",mid="#FFFAFA",high = "#003399",midpoint = 0, aesthetics = c("fill","col"), name = "Difference runoff 1995-2050\n under SSP370 in mm/y", limits = limits.diff.runoff)+
  theme_minimal(base_size = 6)+
  theme(legend.position = "bottom")
...
all.temp <- c(wr.sf.126.2050$Temp-wr.sf.126.2050$Temp95,wr.sf.370.2050$Temp-wr.sf.370.2050$Temp95, wr.sf.126.2100$Temp-wr.sf.126.2100$Temp95, wr.sf.370.2100$Temp-wr.sf.370.2100$Temp95)
all.prec <- c(wr.sf.126.2050$Precip/wr.sf.126.2050$Prec95*100-100,wr.sf.370.2050$Precip/wr.sf.370.2050$Prec95*100-100, wr.sf.126.2100$Precip/wr.sf.126.2100$Prec95*100-100, wr.sf.370.2100$Precip/wr.sf.370.2100$Prec95*100-100)
...
all.runoff <- c(wr.sf.126.2050$logRunoff-wr.sf.126.2050$logRunoff95,wr.sf.370.2050$logRunoff-wr.sf.370.2050$logRunoff95, wr.sf.126.2100$logRunoff-wr.sf.126.2100$logRunoff95, wr.sf.370.2100$logRunoff-wr.sf.370.2100$logRunoff95)

# Temp -----
temp.pal <-  brewer.pal(name = "RdYlBu", n = 9)[c(1,5,9)]

t1 <- ggplot(wr.sf.126.2050)+geom_sf(aes(fill = Temp-Temp95, col = Temp-Temp95)) + 
