...
fennoscandia.ndvi.model <- cbind(fennoscandia.spdf,quadra.lm.ndvi$residuals)
...
resi.plot <- ggplot(st_as_sf(fennoscandia.ndvi.model))+geom_sf(aes(fill = quadra.lm.ndvi$residuals, col = quadra.lm.ndvi$residuals))+
  scale_color_gradient2(high = "darkgreen", low = "red", mid = "white", midpoint = 0, aesthetics = c("fill", "col"))+theme_classic(base_size = 6)+theme(legend.position = "bottom")
...
abline(a = 0, b = 1)
saveRDS(ndvi.model.high, "WR/ndvi.model.high.rds")
...
abline(a = 0, b = 1)
saveRDS(ndvi.model.low, "WR/ndvi.model.low.rds")


# low altitude
wr.n.s.la <- knearneigh(filter(wr.centroid, Alt < 200), k = 100)
wr.n.nb.la <- knn2nb(wr.n.s.la)
wr.kmat.la <- nb2listw(wr.n.nb.la)
ndvi.model.la <- errorsarlm(formula = NDVI~Temp+log(Precip)+log(Alt), data = filter(wr.sf.95, Alt < 200), listw = wr.kmat.la)
plot(filter(wr.sf.95,Alt < 200)$NDVI, ndvi.model.la$fitted.values)
abline(a = 0, b = 1)

wr.n.s.ha <- knearneigh(filter(wr.centroid, Alt > 200), k = 100)
wr.n.nb.ha <- knn2nb(wr.n.s.ha)
wr.kmat.ha <- nb2listw(wr.n.nb.ha)
ndvi.model.ha <- errorsarlm(formula = NDVI~Temp+log(Precip)+log(Alt), data = filter(wr.sf.95, Alt > 200), listw = wr.kmat.ha)
plot(filter(wr.sf.95,Alt > 200)$NDVI, ndvi.model.ha$fitted.values)
abline(a = 0, b = 1)
...
f <- (x-a)^(p-1)*(b-x)^(q-1)/(beta(p,q)*(b-a)^(p+q+1))
f.ndvi <- nls(NDVI ~ exp(-(Temp-b)^2)+exp(-(log(Alt)-u)^2), data = wr.sf.95, start = c(b = 2, u = 4))
plot(wr.sf.95$NDVI, fitted.values(f.ndvi))
