...
palettes <- data.frame(diffTOC =  brewer.pal(name = "RdBu", n = 9)[c(9,5,1)],
                       diffTOC.percent = brewer.pal(name = "RdBu", n = 9)[c(9,5,1)])
...
ggplot(wr.norge)+geom_point(aes(x=Forest,y = forest_increase))
mean(wr.norge$forest_increase)

wr.sweden$forest_increase <- dbeta(wr.sweden$Forest,shape1 = 3, shape2 = 7)/1000
ggplot(wr.sweden)+geom_point(aes(x=Forest,y = forest_increase))
mean(wr.sweden$forest_increase)

wr.finland$forest_increase <- dbeta(wr.finland$Forest,shape1 = 2, shape2 = 7)/100
ggplot(wr.finland)+geom_point(aes(x=Forest,y = forest_increase))
...
wr.sf.forest <- merge(wr.sf.95, select(st_drop_geometry(wr.forest.total), c("gid","forest_increase","Nation"))) %>% merge(select(st_drop_geometry(sem.pred.forest.wr), c("gid","wr.pred.fit","TOC95")), by = "gid")
...
#forest.increase.plot <- cowplot::plot_grid(plotlist = list(forest.increase.norge, forest.increase.sweden,forest.increase.finland), ncol = 3)
#save_plot(plot = forest.increase.plot, filename = "WR/forest_increase.png")
...
wr.forest.long <- wr.forest.pred %>% st_drop_geometry() %>% select(c("gid","Nation","Forest95","Forest","forest_increase")) %>% melt(id.vars = c("gid","Nation"), value.name = "Proportion")
