...
plot_grid(plotlist = listgrob, nrow = 1, ncol = 3, labels= c("a)","b)","c)"), label_size = 25, vjust = 10)
```

`r fig_nums("niva_fit_forest", "a) Observed, b) fitted and c) difference between observed and fitted values for TOC concentration on the 2019-survey data based on the SELM model with Forest, Bogs, and log Runoff as predictors")`
...
`r fig_nums("forest-increase", "Prediction of forest increase with forest policies in a) Norway, b) Sweden, c) Finland")`
...
`r fig_nums("map-increase-forest","Current proportion of forest and estimated potential for increase of forest cover by the year 2050 for a) Norway, b) Sweden and c) Finland")`
...
`r fig_nums("map-toc-forest","Forecast of TOC concentration due to an increase of forest cover as a NbS to abate climate change. a) Modelled TOC with forest increase and b) difference with TOC concentration in 1995")`
