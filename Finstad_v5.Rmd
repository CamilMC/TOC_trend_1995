...
```{r coef-back-transformed}
norge.coef.backtransformed <- norge.coef[-1,1:2]
rownames(norge.coef.backtransformed) <- c("ndvi","runoff","bogs")
...
knitr::kable(norge.coef.backtransformed, digits = 2)
```

```{r spatial-autocorrelation-norway, eval = T}
norge.kmat <- readRDS("norge.kmat.Rdata")
moran.list.norge <- c() 
for (i in c("log.toc","ndvi","log.runoff","asin.sqrt.bogs")){
  moran.list.norge[[i]] <- moran.test(norge[,i],norge.kmat, na.action = na.omit, alternative = "two.sided")
}

moran.df.norge <- data.frame(c("log.toc","ndvi","log.runoff","asin.sqrt.bogs")) %>% setNames("parameter")
moran.df.norge$I <- NA
moran.df.norge$p <- NA
