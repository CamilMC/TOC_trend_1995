...
```{r norge-OLS-relative-change, fig.cap = "Residuals for unscaled OLS"}
...
```{r test-with-unscaling-coefficients, eval = F}
norge.coef.2 <- norge.coef[-1,1:2]


# ndvi: relative change in TOC for 0,1 increase in NDVI
unscaled.ndvi.coef <- norge.coef[2,1]*attr(fennoscandia$s.ndvi,'scaled:scale')+attr(fennoscandia$s.ndvi,'scaled:center')
unscaled.ndvi.se <- norge.coef[2,2]*attr(fennoscandia$s.ndvi,'scaled:scale')+attr(fennoscandia$s.ndvi,'scaled:center')
norge.coef.2[1,1] <- 10^(unscaled.ndvi.coef*0.1) # effect size
norge.coef.2[1,2] <- 0.1*log(10)*10^(unscaled.ndvi.coef*0.1)*unscaled.ndvi.se # delta method: SE(effect size)=d(effect size)/dx * SE(estimate)

#runoff
unscaled.log.runoff.coef <- norge.coef[3,1]*attr(fennoscandia$s.log.runoff,'scaled:scale')+attr(fennoscandia$s.log.runoff,'scaled:center')
unscaled.log.runoff.se <- norge.coef[3,2]*attr(fennoscandia$s.log.runoff,'scaled:scale')+attr(fennoscandia$s.log.runoff,'scaled:center')

norge.coef.2[2,1] <- 1.1^(unscaled.log.runoff.coef) #log-log
norge.coef.2[2,2] <- 1.1^(unscaled.log.runoff.coef)*log(1.1)*unscaled.log.runoff.se

# bogs
unscaled.bogs.coef <- norge.coef[4,1]*attr(fennoscandia$s.bogs,'scaled:scale')+attr(fennoscandia$s.bogs,'scaled:center')
unscaled.bogs.se <- norge.coef[4,2]*attr(fennoscandia$s.bogs,'scaled:scale')+attr(fennoscandia$s.bogs,'scaled:center')

norge.coef.2[3,1] <- 10^(unscaled.bogs.coef) # effect size
norge.coef.2[3,2] <- 0.1*log(10)*10^(unscaled.bogs.coef*0.1)*unscaled.bogs.se 
```
...
fm.s <- s.log.toc~s.ndvi+s.log.runoff+s.log.prec+s.temp+s.bogs+s.arable+s.tndep
s.lm.fennoscandia <- lm(fm.s, data = fennoscandia, na.action = na.exclude)

fm <- log.toc~ndvi.value+log.runoff+log.prec+temp+bogs+arable+tndep
...
summary.table <- cbind(select(lm.coef,Parameter),s.lm.coef[,2:3],lm.coef[,2:3],lm.effectsize[,2:4]) %>%
...
summary.table[,-1] <- apply(summary.table[,-1],2,as.numeric)

knitr::kable(summary.table,digits = 3) %>%
...
```{r lm-relative-change}
k.neigh.w <- readRDS("k.neigh.w.Rdata")
fm <- log.toc~ndvi.value+log.runoff+log.prec+temp+bogs+arable+tndep
lm.fennoscandia.rc <- lm(fm, data = fennoscandia, na.action = na.exclude)
summary(lm.fennoscandia.rc)
lm.rc.r2 <- summary(lm.fennoscandia.rc)$adj.r.squared
moran.I.res.lm.rc <- lm.morantest(lm.fennoscandia.rc, k.neigh.w, alternative = "two.sided")
moran.limit.fennoscandia <- 1/(dim(fennoscandia)[1]-1)

map <- f_plotspatial(data = fennoscandia, var = lm.fennoscandia.rc$residuals,plottitle = "OLS residuals")+annotate("label",x = 8, y = 69, label = paste("Mean R2 =",round(lm.rc.r2,2)))+annotate("label",x=8,y=68, label = paste("AIC = ", round(AIC(lm.fennoscandia.rc),2)))+annotate("label",x=8,y=66.5,label = paste("Moran's I of \n residuals = ", round(moran.I.res.lm.rc$estimate[1],3)))
plot(map)

lm.coef.rc <- summary(lm.fennoscandia.rc)$coefficients %>% as.data.frame()
lm.coef.rc <- lm.coef.rc %>% rownames_to_column(var = "exp.var")


lm.coef.backtransformed <- lm.coef.rc[-1,1:3]
names(lm.coef.backtransformed) <- c("parameter","effect.size","std.error")

i <- 2 # estimate column in lm.coef.rc
j <- 3 # se column in lm.coef.rc


# ndvi
lm.coef.backtransformed[1,2] <- 100*(10^(lm.coef.rc[1,i]*0.1)-1) # effect size
lm.coef.backtransformed[1,3] <- 0.1*log(10)*10^(lm.coef.rc[2,i]*0.1)*lm.coef.rc[2,j] # delta method: SE(effect size)=d(effect size)/dx * SE(estimate)

# log.runoff
lm.coef.backtransformed[2,2] <- 100*(1.1^(lm.coef.rc[3,i])-1) #log-log
lm.coef.backtransformed[2,3] <- 1.1^(lm.coef.rc[3,i])*log(1.1)*lm.coef.rc[3,j]

# log.prec
lm.coef.backtransformed[3,2] <- 100*(1.1^(lm.coef.rc[4,i])-1) #log-log
lm.coef.backtransformed[3,3] <- 1.1^(lm.coef.rc[4,i])*log(1.1)*lm.coef.rc[4,j]

# temp
lm.coef.backtransformed[4,2] <- 100*(10^(lm.coef.rc[5,i]*1)-1) # effect size
lm.coef.backtransformed[4,3] <- 0.1*log(10)*10^(lm.coef.rc[5,i]*4)*lm.coef.rc[5,j]

# bogs
lm.coef.backtransformed[5,2] <- 100*(10^(lm.coef.rc[6,i]*0.01)-1) # effect size
lm.coef.backtransformed[5,3] <- 0.1*log(10)*10^(lm.coef.rc[6,i]*0.01)*lm.coef.rc[6,j]

# arable
lm.coef.backtransformed[6,2] <- 100*(10^(lm.coef.rc[7,i]*0.01)-1) # effect size
lm.coef.backtransformed[6,3] <- 0.1*log(10)*10^(lm.coef.rc[7,i]*0.01)*lm.coef.rc[7,j]

# tndep
lm.coef.backtransformed[7,2] <- 100*(10^(lm.coef.rc[8,i]*0.3)-1) # effect size
lm.coef.backtransformed[7,3] <- 0.1*log(10)*10^(lm.coef.rc[8,i]*0.1)*lm.coef.rc[8,j]

lm.coef.backtransformed$unit <- c("% change of TOC concentration for a 0.01 increase in NDVI",
                                         "% change of TOC concentration for a 10% increase in runoff",
                                         "% change of TOC concentration for a 10% increase in precipitations",
                                         "% change of TOC concentration for a 4 degree increase in temperature",
                                         "% change of TOC concentration for a 1% increase in proportion of bogs",
                                         "% change of TOC concentration for a 1% increase in proportion of arable land",
                                         "% change of TOC concentration for a 10 ug decrease of N-dep / m2")
coef.plot.backtransformed <- ggplot(lm.coef.backtransformed)+
  geom_col(aes(x=effect.size,y=parameter,fill=effect.size))+
  theme_light()+
...
fm.s <- s.log.toc~s.ndvi+s.log.runoff+s.log.prec+s.temp+s.bogs+s.arable+s.
sem.fennoscandia.s <- errorsarlm(fm.s,fennoscandia,k.neigh.w)
...
fm <- s.log.toc~s.ndvi+s.log.runoff+s.log.prec+s.temp+s.bogs+s.arable+s.tndep
library(GWmodel)
...
```{r gwss, eval = F, fig.dim= c(15,10)}
bw.fennoscandia <- readRDS("bw.fennoscandia.Rdata")
variables <- c("log.toc","ndvi.value","log.prec","log.runoff","temp","slope","bogs","arable","tndep")
...
f_plotspatial(fennoscandia,var = gwss.fennoscandia$SDF$Corr_log.toc.temp,plottitle = "Correlation TOC-NDVI")
```
