...
For the non-transformed independent variables (NDVI,percentage of bogs and arable land, N-deposition), the effect size was calculated taking into account that the dependent variable was logged, using *natural log* (here noted as "log"). 

$\log([TOC]_1) = \hat{\theta} x_1$

$\log([TOC]_2) = \hat{\theta} x_2$

$\log([TOC]_2) - \ln ([TOC]_1) = \hat{\theta}  (x_2 - x_1)$
...
The space-for-time forecast, based on the Spatial Error Linear Model, predict the future log(TOC). Therefore, the maps showing the difference of log in the future show the %change of TOC between 1995 and the given SSP: 

$Difference \; log([TOC]) = log([TOC]_{forecast})-log([TOC]_{initial}) = log(\frac{[TOC]_{forecast}}{[TOC]_{initial}})$

and 
 
$exp(log([TOC]_{forecast})-log([TOC]_{initial})) = \frac{[TOC]_{forecast}}{[TOC]_{initial}}$

Therefore:

$[TOC]_{forecast} = [TOC]_{initial} \times exp(difference \; log([TOC]))$

The percent change is: 

$\delta[TOC]\% = ([TOC]_{forecast}/[TOC]_{initial}) -1) \times 100$
